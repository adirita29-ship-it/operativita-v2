...
export default function App() {
  ...
  const emptyInc=(cat="vendita")=>({categoria:cat,agenteListing:"",percListing:0,buyerListing:"",percBuyerListing:0,fonte:"",nominativo:"",comune:"",indirizzo:"",tipologia:"",dataInizio:todayStr(),scadenza:"",prezzoRichiesto:"",prezzoReale:"",provvPrevista:"",note:"",stato:"Attivo",archiviato:false});
  const salvaInc=()=>{
  ...
  const salvaStatoProp=()=>{
    ...
    if(ns==="Accettata") upd.dataVendita=oggi;
    // Se vincolo positivo → data vendita oggi
    if(ns==="Accettata con Vincolo"&&formStatoProp.esitoVincolo==="Positivo") upd.dataVendita=oggi;
                ...
                <table style={{width:"100%",borderCollapse:"collapse"}}>
                  ...
                  <tbody>{propVincolo.map(p=>(
                    <tr key={p.id}>
                      ...
                      <td style={S.tdS}><button style={{...S.btnP,fontSize:11,padding:"3px 10px",background:"#D4AC0D",borderColor:"#D4AC0D"}} onClick={()=>{setFormStatoProp({stato:p.stato,noteStato:"",esitoVincolo:"",tipoNegazione:""});setShowGestProp(p);}}>Gestisci</button></td>
                    </tr>
                  ))}</tbody>
                  <tfoot><tr style={{background:BRAND.beige,fontWeight:500}}>
            ...
            <div style={S.tblWrap}><table style={S.tbl}>
              ...
              <tbody>{incFiltrati.map(inc=>{
                ...
                return(<tr key={inc.id} style={{background:inc.archiviato?"#fafafa":"white",opacity:inc.archiviato?0.7:1}}>
                  ...
                  <td style={S.tdR}>€ {fmtN(inc.prezzoRichiesto)}</td>
      ...
      {showInc&&(<div style={S.overlay} onClick={e=>e.target===e.currentTarget&&setShowInc(null)}>
        <div style={S.modal}>
          ...
          <div style={S.g2}>
            ...
            <div><label style={S.lbl}>{formInc.categoria==="affitto"?"Canone mensile (EUR)":"Prezzo richiesto (EUR)"}</label><input style={S.inp} type="number" value={formInc.prezzoRichiesto||""} onChange={e=>setFormInc({...formInc,prezzoRichiesto:e.target.value})}/></div>
            ...
            <div><label style={S.lbl}>Provvigione prevista (EUR)</label><input style={S.inp} type="number" value={formInc.provvPrevista||""} onChange={e=>setFormInc({...formInc,provvPrevista:e.target.value})}/></div>
          </div>
      ...
      {showProp&&(<div style={S.overlay} onClick={e=>e.target===e.currentTarget&&setShowProp(null)}>
        <div style={S.modal}>
          ...
          <div style={S.g2}>
            ...
            <div><label style={S.lbl}>% Provv. Acquirente</label><input style={S.inp} type="number" step="0.1" value={formProp.percProvvAcquirente||""} onChange={e=>{const perc=Number(e.target.value);setFormProp({...formProp,percProvvAcquirente:e.target.value,provvAcquirente:formProp.prezzoOfferto?Math.round(Number(formProp.prezzoOfferto)*perc/100):formProp.provvAcquirente});}}/></div>
            <div><label style={S.lbl}>Provv. acquirente (EUR) — calcolata o manuale</label><input style={S.inp} type="number" value={formProp.provvAcquirente||""} onChange={e=>setFormProp({...formProp,provvAcquirente:e.target.value})}/></div>
            ...
            <div><label style={S.lbl}>Provv. venditore (EUR)</label><input style={S.inp} type="number" value={formProp.provvVenditore||""} onChange={e=>setFormProp({...formProp,provvVenditore:e.target.value})}/></div>
          </div>
