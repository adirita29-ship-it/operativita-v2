...
const Logo = ({size=24,light=false}) => (
  <div style={{lineHeight:1}}>
    <div style={{fontSize:size,fontWeight:700,color:light?"#fff":BRAND.grigio,letterSpacing:"-0.5px",fontFamily:"Georgia,serif"}}>c<span style={{color:light?"rgba(255,255,255,0.85)":BRAND.oro}}>à</span>sa</div>
    <div style={{fontSize:size*0.33,letterSpacing:"0.25em",color:light?"rgba(255,255,255,0.9)":BRAND.grigio,borderTop:`1px solid ${light?"rgba(255,255,255,0.6)":BRAND.grigio}`,paddingTop:2,marginTop:1}}>IMMOBILIARE</div>
  </div>
);
...
function LoginPage({onLogin}) {
  ...
  const handleLogin=()=>{
    ...
    setTimeout(()=>{
      ...
      if(user){onLogin(user);}else{setErrore("Email o password non corretti.");setLoading(false);}
    },600);
  };
  return(
    <div style={{minHeight:"100vh",background:`linear-gradient(135deg, ${BRAND.oro} 0%, #A8863A 100%)`,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"2rem"}}>
      <div style={{textAlign:"center",marginBottom:"2.5rem"}}><Logo size={64} light={true}/></div>
      <div style={{background:"#fff",borderRadius:16,padding:"2rem 2.5rem",width:"min(90vw,380px)",boxShadow:"0 20px 60px rgba(0,0,0,0.2)"}}>
        ...
        <div style={{marginBottom:12}}>
          ...
          <input style={{width:"100%",fontSize:14,padding:"10px 12px",borderRadius:8,border:`1.5px solid ${errore?"#e74c3c":"#ddd"}`,boxSizing:"border-box",color:BRAND.grigio,outline:"none"}} type="email" value={email} onChange={e=>{setEmail(e.target.value);setErrore("");}} placeholder="email@casaimmobiliarevarese.it" onKeyDown={e=>e.key==="Enter"&&handleLogin()}/>
        </div>
        <div style={{marginBottom:errore?8:20}}>
          <label style={{fontSize:12,color:"#999",display:"block",marginBottom:4}}>Password</label>
          <input style={{width:"100%",fontSize:14,padding:"10px 12px",borderRadius:8,border:`1.5px solid ${errore?"#e74c3c":"#ddd"}`,boxSizing:"border-box",color:BRAND.grigio,outline:"none"}} type="password" value={password} onChange={e=>{setPassword(e.target.value);setErrore("");}} placeholder="••••••••" onKeyDown={e=>e.key==="Enter"&&handleLogin()}/>
        </div>
        {errore&&<p style={{fontSize:12,color:"#e74c3c",margin:"0 0 16px",textAlign:"center"}}>{errore}</p>}
        <button onClick={handleLogin} style={{width:"100%",padding:"12px",fontSize:15,fontWeight:600,borderRadius:8,border:"none",background:`linear-gradient(135deg, ${BRAND.oro}, #A8863A)`,color:"#fff",cursor:"pointer",opacity:loading?0.7:1}}>{loading?"Accesso in corso...":"Accedi"}</button>
      </div>
...
export default function App() {
  ...
  const filterAM=(p,anno,mese)=>{if(anno!=="Tutti"&&getAnno(p.dataProposta)!==anno) return false;if(mese!=="Tutti"&&p.dataProposta!==mese) return false;return true;};
  ...
  const totalePrevisionale=useMemo(()=>costiAnnoCorrente.reduce((s,c)=>{const imp=Number(c.importoPrevisionale||0);return s+(c.frequenza==="Mensile"?imp*12:imp);},0),[costiAnnoCorrente]);
  const totaleConsuntivo=useMemo(()=>costiAnnoCorrente.reduce((s,c)=>s+Object.values(c.consuntivi||{}).reduce((a,v)=>a+Number(v||0),0),0),[costiAnnoCorrente]);
  const quotaNettoAgenzia=useMemo(()=>proposte.filter(p=>p.stato==="Incassato"&&getAnno(p.dataProposta)===costiAnno).reduce((s,p)=>{const q=calcolaQuote(p,agenti);return s+q.quotaAgenziaL+q.quotaAgenziaA;},0),[proposte,agenti,costiAnno]);
  ...
  const salvaCosto=()=>{
    ...
    const ac=costi[costiAnno]||[];
    if(editCostoId){setCosti({...costi,[costiAnno]:ac.map(c=>c.id===editCostoId?{...c,...formCosto}:c)});}
    else{setCosti({...costi,[costiAnno]:[...ac,{id:Date.now(),...formCosto,consuntivi:{}}]});}
  ...
  const aggiungiAnno=anno=>{if(!anno||costi[anno]) return;setCosti({...costi,[anno]:mkCostoAnno()});setObiettivi({...obiettivi,[anno]:{fatturato:0,compensoTarget:0}});setCostiAnno(anno);};
  ...
  return (
    <div style={S.wrap}>
      ...
      {tab==="Dashboard"&&(<div style={S.sec}><div style={S.fRow}><AnnoSel value={dashAnno} onChange={setDashAnno}/></div><div style={S.g3}><DashCard label="Incassato" tot={dashboard.incassato.tot} clr={STATI_CFG.Incassato.clr} sub={[["Agenzia",dashboard.incassato.agenzia],["Agenti",dashboard.incassato.agenti],["Buyer",dashboard.incassato.buyer]]}/><DashCard label="Venduto — da incassare" tot={dashboard.venduto.tot} clr={STATI_CFG.Accettata.clr} sub={[["Agenzia",dashboard.venduto.agenzia],["Agenti",dashboard.venduto.agenti],["Buyer",dashboard.venduto.buyer]]}/><DashCard label="In trattativa" tot={dashboard.inCorso.tot} clr={STATI_CFG.Proposta.clr} sub={[["Agenzia",dashboard.inCorso.agenzia],["Agenti",dashboard.inCorso.agenti],["Buyer",dashboard.inCorso.buyer]]}/></div><div style={S.prevBox}><div style={{display:"flex",gap:"2rem",flexWrap:"wrap"}}>{[["Previsionale",`€ ${fmt(incassTot+vendTot)}`],["Pratiche attive",dashProposte.filter(p=>!["Rifiutata","Incassato","Mancata Chiusura"].includes(p.stato)).length],["Incassate",dashProposte.filter(p=>p.stato==="Incassato").length],["Con vincolo",dashProposte.filter(p=>p.stato==="Accettata con Vincolo").length],["Non concluse",dashProposte.filter(p=>STATI_CHIUSI.includes(p.stato)).length]].map(([l,v])=>(<div key={l}><p style={{fontSize:12,color:"#aaa",margin:"0 0 2px"}}>{l}</p><p style={{fontSize:20,fontWeight:500,margin:0}}>{v}</p></div>))}</div></div><div style={S.tblWrap}><table style={{...S.tbl,minWidth:600}}><thead><tr>{["Agente","Profilo","Pratiche inc.","Produzione","Inc. Buyer","Quota agente","Quota agenzia"].map(h=><th key={h} style={S.th}>{h}</th>)}</tr></thead><tbody>{agenti.map(ag=>{const pInc=dashProposte.filter(p=>p.stato==="Incassato"&&(p.agenteListing===ag.id||p.agenteAcquirente===ag.id||p.buyer===ag.id||p.buyerL===ag.id));let prod=0,qa=0,ib=0;pInc.forEach(p=>{const q=calcolaQuote(p,agenti);if(p.agenteListing===ag.id){prod+=q.tv;if(ag.profilo!=="Broker")qa+=q.quotaListingAgente;}if(p.agenteAcquirente===ag.id){prod+=q.ta;if(ag.profilo!=="Broker")qa+=q.quotaAcquirenteAgente;}if(p.buyerL===ag.id&&p.agenteListing!==ag.id)ib+=q.quotaBuyerL;if(p.buyer===ag.id&&p.agenteAcquirente!==ag.id)ib+=q.quotaBuyer;});return(<tr key={ag.id}><td style={S.td}><strong>{ag.nome} {ag.cognome}</strong></td><td style={S.td}>{ag.profilo}</td><td style={S.td}>{pInc.length}</td><td style={S.tdR}>€ {fmt(prod)}</td><td style={S.tdR}>{ib>0?`€ ${fmt(ib)}`:"—"}</td><td style={S.tdR}>{ag.profilo==="Broker"?"—":`€ ${fmt(qa)}`}</td><td style={S.tdR}>€ {fmt(prod-qa)}</td></tr>);})}</tbody></table></div></div>)}

      {tab==="Proposte"&&(<div style={S.sec}><div style={S.fRow}><AnnoSel value={fAnno} onChange={setFAnno}/><MeseSel value={fMese} onChange={setFMese} anno={fAnno}/><select style={S.sel} value={fStato} onChange={e=>setFStato(e.target.value)}><option value="Tutti">Tutti gli stati</option>{Object.keys(STATI_CFG).map(s=><option key={s}>{s}</option>)}</select><select style={S.sel} value={fAgente} onChange={e=>setFAgente(e.target.value)}><option value="Tutti">Tutti gli agenti</option>{agenti.map(a=><option key={a.id} value={a.id}>{a.nome} {a.cognome}</option>)}</select><div style={{flex:1}}/><button style={S.btnP} onClick={()=>{setEditPropId(null);setForm(emptyForm());setShowProp(true);}}>+ Nuova proposta</button></div><div style={S.tblWrap}><table style={S.tbl}><thead><tr>{["Data prop.","Ag. Listing","Buyer L","Ag. Acquirente","Buyer","Comune / Indirizzo","Venditore","Acquirente","Prezzo offerto","Prezzo accettato","Provv. V.","Provv. A.","Vincolo","Stato","Azioni"].map(h=><th key={h} style={S.th}>{h}</th>)}</tr></thead><tbody>{propFiltrate.map(p=>{const q=calcolaQuote(p,agenti);const cfg=STATI_CFG[p.stato]||{clr:"#888"};const prossimi=PROSSIMI[p.stato]||[];return(<tr key={p.id} style={{background:STATI_CHIUSI.includes(p.stato)?"#FDF8F8":undefined}}><td style={S.td}><strong>{p.dataProposta}</strong></td><td style={S.td}>{nomAg(p.agenteListing)}<br/><span style={{fontSize:11,color:"#bbb"}}>{p.percListingConf!==undefined?`${p.percListingConf}% ✓`:p.percListing+"%"}</span></td><td style={S.td}>{p.buyerL?<>{nomAg(p.buyerL)}<br/><span style={{fontSize:11,color:"#bbb"}}>{p.percBuyerL}%</span></>:"—"}</td><td style={S.td}>{nomAg(p.agenteAcquirente)}<br/><span style={{fontSize:11,color:"#bbb"}}>{p.percAcquirenteConf!==undefined?`${p.percAcquirenteConf}% ✓`:p.percAcquirente+"%"}</span></td><td style={S.td}>{p.buyer?<>{nomAg(p.buyer)}<br/><span style={{fontSize:11,color:"#bbb"}}>{p.percBuyer}%</span></>:"—"}</td><td style={S.td}><strong>{p.comune}</strong><br/><span style={{fontSize:11,color:"#aaa"}}>{p.indirizzo}·{p.tipologia}</span></td><td style={S.td}>{p.venditore}</td><td style={S.td}>{p.acquirente}</td><td style={S.tdR}>€ {fmtN(p.prezzoVendita)}</td><td style={S.tdR}>{p.prezzoAccettato?<strong style={{color:BRAND.oroD}}>€ {fmtN(p.prezzoAccettato)}</strong>:<span style={{color:"#ccc"}}>—</span>}</td><td style={S.tdR}>€ {fmt(q.tv)}</td><td style={S.tdR}>€ {fmt(q.ta)}</td><td style={S.td}>{p.vincolata?<><span style={{fontSize:11,fontWeight:500,color:BRAND.oroD}}>{p.tipoVincolo}</span><br/><span style={{fontSize:10,color:"#aaa"}}>{p.dataVincolo}</span></>:<span style={{color:"#ccc",fontSize:11}}>—</span>}</td><td style={S.td}><span style={badgeStyle(p.stato)}>{cfg.semaforo} {cfg.label||p.stato}</span>{p.noteStato&&<div style={{fontSize:11,color:"#aaa",marginTop:2,maxWidth:110,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}} title={p.noteStato}>{p.noteStato}</div>}</td><td style={S.td}><div style={{display:"flex",gap:4,flexWrap:"wrap"}}>{prossimi.length>0&&<button style={S.btnP} onClick={()=>apriAvanza(p)}>Avanza →</button>}<button style={S.btn} onClick={()=>apriEditProp(p)}>✎</button><button style={S.btn} onClick={()=>setShowStorico(p)}>⏱</button><button style={S.btnDanger} onClick={()=>eliminaProp(p.id)}>✕</button></div></td></tr>);})}{propFiltrate.length===0&&<tr><td colSpan={15} style={{...S.td,textAlign:"center",color:"#bbb",padding:"2rem"}}>Nessuna proposta trovata</td></tr>}</tbody>{propFiltrate.length>0&&(<tfoot><tr style={S.totRow}><td colSpan={9} style={S.td}>Totale ({propFiltrate.length})</td><td style={S.td}/><td style={S.tdR}>€ {fmt(propFiltrate.reduce((s,p)=>s+calcolaQuote(p,agenti).tv,0))}</td><td style={S.tdR}>€ {fmt(propFiltrate.reduce((s,p)=>s+calcolaQuote(p,agenti).ta,0))}</td><td colSpan={3} style={S.td}><strong>Tot: € {fmt(propFiltrate.reduce((s,p)=>s+totProvv(p),0))}</strong></td></tr></tfoot>)}</table></div></div>)}

      {tab==="Venduti"&&(<div style={S.sec}><div style={S.fRow}><AnnoSel value={vendAnno} onChange={setVendAnno}/><MeseSel value={vendMese} onChange={setVendMese} anno={vendAnno}/><div style={{flex:1}}/><span style={{fontSize:13,color:"#aaa"}}>Accettate · Preliminare · Rogito · Incassate</span></div><div style={S.tblWrap}><table style={S.tbl}><thead><tr>{["Data","Ag. Listing","BuyerL","Ag. Acq.","Buyer","Immobile","Venditore","Acquirente","Prezzo offerto","Prezzo accettato","% List.","Provv.V.","BuyerL€","Netto V.","% Acq.","Provv.A.","Buyer€","Netto A.","Tipo atto","Data atto","Acconto","Stato"].map(h=><th key={h} style={S.th}>{h}</th>)}</tr></thead><tbody>{venduti.map(p=>{const q=calcolaQuote(p,agenti);return(<tr key={p.id}><td style={S.td}>{p.dataProposta}</td><td style={S.td}>{nomAg(p.agenteListing)}</td><td style={S.td}>{p.buyerL?nomAg(p.buyerL):"—"}</td><td style={S.td}>{nomAg(p.agenteAcquirente)}</td><td style={S.td}>{p.buyer?nomAg(p.buyer):"—"}</td><td style={S.td}><strong>{p.comune}</strong><br/><span style={{fontSize:11,color:"#aaa"}}>{p.indirizzo}·{p.tipologia}</span></td><td style={S.td}>{p.venditore}</td><td style={S.td}>{p.acquirente}</td><td style={S.tdR}>€ {fmtN(p.prezzoVendita)}</td><td style={{...S.tdR,fontWeight:500,color:BRAND.oroD}}>€ {fmtN(p.prezzoAccettato||p.prezzoVendita)}</td><td style={S.tdR}>{p.percListingConf??p.percListing}%</td><td style={S.tdR}>€ {fmt(q.tv)}</td><td style={S.tdR}>{q.quotaBuyerL>0?`€ ${fmt(q.quotaBuyerL)}`:"—"}</td><td style={{...S.tdR,fontWeight:500}}>€ {fmt(q.quotaAgenziaL)}</td><td style={S.tdR}>{p.percAcquirenteConf??p.percAcquirente}%</td><td style={S.tdR}>€ {fmt(q.ta)}</td><td style={S.tdR}>{q.quotaBuyer>0?`€ ${fmt(q.quotaBuyer)}`:"—"}</td><td style={{...S.tdR,fontWeight:500}}>€ {fmt(q.quotaAgenziaA)}</td><td style={S.td}><span style={{fontSize:12,fontWeight:500,color:p.tipoAtto==="Rogito Diretto"?BRAND.oroD:BRAND.grigio}}>{p.tipoAtto||"—"}</span></td><td style={S.td}><span style={{fontSize:12,color:"#aaa"}}>{p.dataAtto||"—"}</span></td><td style={S.tdR}>{Number(p.accontoProvv)>0?`€ ${fmt(p.accontoProvv)}`:"—"}</td><td style={S.td}><span style={badgeStyle(p.stato)}>{STATI_CFG[p.stato]?.semaforo} {STATI_CFG[p.stato]?.label||p.stato}</span></td></tr>);})}{venduti.length===0&&<tr><td colSpan={22} style={{...S.td,textAlign:"center",color:"#bbb",padding:"2rem"}}>Nessun venduto trovato</td></tr>}</tbody>{venduti.length>0&&(<tfoot><tr style={S.totRow}><td colSpan={11} style={S.td}>Totale ({venduti.length})</td><td style={S.tdR}>€ {fmt(venduti.reduce((s,p)=>s+calcolaQuote(p,agenti).tv,0))}</td><td style={S.tdR}>€ {fmt(venduti.reduce((s,p)=>s+calcolaQuote(p,agenti).quotaBuyerL,0))}</td><td style={S.tdR}>€ {fmt(venduti.reduce((s,p)=>s+calcolaQuote(p,agenti).quotaAgenziaL,0))}</td><td style={S.td}/><td style={S.tdR}>€ {fmt(venduti.reduce((s,p)=>s+calcolaQuote(p,agenti).ta,0))}</td><td style={S.tdR}>€ {fmt(venduti.reduce((s,p)=>s+calcolaQuote(p,agenti).quotaBuyer,0))}</td><td style={S.tdR}>€ {fmt(venduti.reduce((s,p)=>s+calcolaQuote(p,agenti).quotaAgenziaA,0))}</td><td colSpan={4} style={S.td}/></tr></tfoot>)}</table></div></div>)}

      {tab==="Report Agenti"&&(<div style={S.sec}><div style={S.fRow}><AnnoSel value={rAnno} onChange={setRAnno}/><MeseSel value={rMese} onChange={setRMese} anno={rAnno}/><select style={S.sel} value={rAgente} onChange={e=>setRAgente(e.target.value)}><option value="Tutti">Tutti gli agenti</option>{agenti.map(a=><option key={a.id} value={a.id}>{a.nome} {a.cognome}</option>)}</select></div><div style={S.tblWrap}><table style={{...S.tbl,minWidth:600}}><thead><tr>{["Agente","Profilo","Tipo","Pratiche","Produzione","% Ag.","Quota agente","Quota agenzia","Inc. come Buyer"].map(h=><th key={h} style={S.th}>{h}</th>)}</tr></thead><tbody>{reportData.map(a=>(<tr key={a.id}><td style={S.td}><strong>{a.nome} {a.cognome}</strong></td><td style={S.td}>{a.profilo}</td><td style={S.td}>{a.tipo}</td><td style={S.td}>{a.nPratiche}</td><td style={S.tdR}>€ {fmt(a.produzione)}</td><td style={S.td}>{a.profilo==="Broker"?"—":`${a.percListing}%`}</td><td style={S.tdR}>{a.profilo==="Broker"?"—":`€ ${fmt(a.quotaAg)}`}</td><td style={S.tdR}>€ {fmt(a.quotaAgenzia)}</td><td style={S.tdR}>{a.incassatoBuyer>0?`€ ${fmt(a.incassatoBuyer)}`:"—"}</td></tr>))}</tbody>{reportData.length>0&&(<tfoot><tr style={S.totRow}><td colSpan={4} style={S.td}>Totale</td><td style={S.tdR}>€ {fmt(reportData.reduce((s,a)=>s+a.produzione,0))}</td><td style={S.td}/><td style={S.tdR}>€ {fmt(reportData.reduce((s,a)=>s+a.quotaAg,0))}</td><td style={S.tdR}>€ {fmt(reportData.reduce((s,a)=>s+a.quotaAgenzia,0))}</td><td style={S.tdR}>€ {fmt(reportData.reduce((s,a)=>s+a.incassatoBuyer,0))}</td></tr></tfoot>)}</table></div></div>)}

      {tab==="Fattura Agente"&&(<div style={S.sec}><div style={S.fRow}><select style={S.sel} value={fatAgente} onChange={e=>setFatAgente(e.target.value)}><option value="">Seleziona agente...</option>{agentiFattura.map(a=><option key={a.id} value={a.id}>{a.nome} {a.cognome} — {a.profilo}</option>)}</select><AnnoSel value={fatAnno} onChange={setFatAnno}/><MeseSel value={fatMese} onChange={setFatMese} anno={fatAnno}/><div style={{flex:1}}/>{fatturaDati.length>0&&<button style={S.btnP} onClick={()=>window.print()}>🖨 Stampa</button>}</div>{!fatAgente&&<div style={{textAlign:"center",padding:"3rem",color:"#bbb"}}>Seleziona un agente</div>}{fatAgente&&fatturaDati.length===0&&<div style={{textAlign:"center",padding:"3rem",color:"#bbb"}}>Nessuna pratica incassata nel periodo</div>}{fatAgente&&fatturaDati.length>0&&(<div style={S.fatturaWrap}><div style={S.fatturaHdr}><Logo/><div style={{textAlign:"right"}}><p style={{fontSize:12,color:"#aaa",margin:"0 0 2px"}}>Nota provvigioni</p><p style={{fontSize:16,fontWeight:500,margin:"0 0 2px"}}>{fatAg?.nome} {fatAg?.cognome}</p><p style={{fontSize:12,color:"#aaa",margin:0}}>{fatMese!=="Tutti"?fmtMese(fatMese):fatAnno!=="Tutti"?`Anno ${fatAnno}`:"Tutto il periodo"} — {new Date().toLocaleDateString("it-IT")}</p></div></div>{fatturaDati.map(({p,righe,totPratica},i)=>(<div key={p.id} style={S.pratCard}><p style={{fontSize:14,fontWeight:500,margin:"0 0 4px"}}>{i+1}. {p.comune} — {p.indirizzo} <span style={{fontSize:12,color:"#aaa",fontWeight:400}}>({p.tipologia})</span></p><p style={{fontSize:12,color:"#aaa",margin:"0 0 .5rem"}}>Prezzo accettato: <strong style={{color:BRAND.oroD}}>€ {fmtN(p.prezzoAccettato||p.prezzoVendita)}</strong></p>{righe.map((r,j)=>(<div key={j} style={S.rigaBox}><p style={S.rigaTipo}>{r.tipo}</p><div style={S.rigaRow}><span style={{color:"#888"}}>Cliente:</span><span style={{fontWeight:500}}>{r.cliente}</span></div><div style={S.rigaRow}><span style={{color:"#888"}}>Provv. agenzia ({r.percAg}%):</span><span>€ {fmt(r.provvAgenzia)}</span></div><div style={S.rigaRow}><span style={{color:"#888"}}>Quota agente:</span><span style={{fontWeight:500,color:BRAND.oroD}}>€ {fmt(r.quotaAg)}</span></div></div>))}<p style={{textAlign:"right",fontSize:13,fontWeight:500,color:BRAND.oroD,marginTop:4}}>Subtotale: € {fmt(totPratica)}</p></div>))}<div style={S.totBox}><span style={{fontSize:14,fontWeight:500,color:"#fff"}}>TOTALE IMPONIBILE DA FATTURARE</span><span style={{fontSize:22,fontWeight:700,color:"#fff"}}>€ {fmt(totaleImponibile)}</span></div></div>)}</div>)}

      {tab==="Costi & Break Even"&&(<div style={S.sec}><div style={S.fRow}>{anniCosti.map(a=><button key={a} style={{...S.btn,fontWeight:a===costiAnno?600:400,borderColor:a===costiAnno?BRAND.oro:"#ccc",color:a===costiAnno?BRAND.oro:BRAND.grigio}} onClick={()=>setCostiAnno(a)}>{a}</button>)}<button style={S.btnOut} onClick={()=>{const a=prompt("Anno da aggiungere:");if(a) aggiungiAnno(a.trim());}}>+ Anno</button><div style={{flex:1}}/><button style={S.btnP} onClick={()=>{setEditCostoId(null);setFormCosto({voce:"",tipo:"Fisso",frequenza:"Mensile",importoPrevisionale:0});setShowFormCosto(true);}}>+ Voce di costo</button></div><div style={{...S.prevBox,marginBottom:"1rem"}}><div style={{display:"flex",gap:"1.5rem",flexWrap:"wrap",alignItems:"flex-end"}}><div><p style={{fontSize:12,color:"#aaa",margin:"0 0 4px"}}>Obiettivo fatturato {costiAnno}</p><div style={{display:"flex",gap:6,alignItems:"center"}}><span style={{fontSize:13}}>€</span><input style={{...S.inp,width:130}} type="number" value={obiettivoAnno.fatturato||""} onChange={e=>setObiettivi({...obiettivi,[costiAnno]:{...obiettivoAnno,fatturato:Number(e.target.value)}})} placeholder="es. 360000"/></div></div><div><p style={{fontSize:12,color:"#aaa",margin:"0 0 4px"}}>Compenso mensile obiettivo</p><div style={{display:"flex",gap:6,alignItems:"center"}}><span style={{fontSize:13}}>€</span><input style={{...S.inp,width:120}} type="number" value={obiettivoAnno.compensoTarget||""} onChange={e=>setObiettivi({...obiettivi,[costiAnno]:{...obiettivoAnno,compensoTarget:Number(e.target.value)}})} placeholder="es. 5000"/></div></div><div style={{flex:1}}/><div style={{textAlign:"right"}}><p style={{fontSize:12,color:"#aaa",margin:"0 0 2px"}}>Totale costi previsionali {costiAnno}</p><p style={{fontSize:22,fontWeight:600,margin:0}}>€ {fmt(totalePrevisionale)}</p></div></div></div><div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:"1.25rem"}}><div style={{...S.card(STATI_CFG.Accettata.clr),padding:"1rem 1.25rem"}}><p style={{fontSize:13,fontWeight:500,margin:"0 0 4px",color:STATI_CFG.Accettata.clr}}>🎯 Break Even Base</p><p style={{fontSize:11,color:"#aaa",margin:"0 0 10px"}}>Quota netta agenzia per coprire tutti i costi</p><div style={{display:"flex",justifyContent:"space-between",marginBottom:4}}><span style={{fontSize:13,color:"#666"}}>Costi totali previsti:</span><span style={{fontSize:13,fontWeight:500}}>€ {fmt(breakEvenBase)}</span></div><div style={{display:"flex",justifyContent:"space-between",marginBottom:4}}><span style={{fontSize:13,color:"#666"}}>Quota netta incassata:</span><span style={{fontSize:13,fontWeight:500,color:STATI_CFG.Incassato.clr}}>€ {fmt(quotaNettoAgenzia)}</span></div><div style={S.progressBar(percBreakEven,STATI_CFG.Accettata.clr)}/><div style={{display:"flex",justifyContent:"space-between",marginTop:6}}><span style={{fontSize:12,color:"#aaa"}}>{percBreakEven}% raggiunto</span><span style={{fontSize:12,fontWeight:500,color:quotaNettoAgenzia>=breakEvenBase?"#27AE60":"#E67E22"}}>{quotaNettoAgenzia>=breakEvenBase?`✅ Superato di € ${fmt(quotaNettoAgenzia-breakEvenBase)}`:`Mancano € ${fmt(breakEvenBase-quotaNettoAgenzia)}`}</span></div></div><div style={{...S.card(BRAND.oro),padding:"1rem 1.25rem"}}><p style={{fontSize:13,fontWeight:500,margin:"0 0 4px",color:BRAND.oroD}}>🏆 Break Even Obiettivo</p><p style={{fontSize:11,color:"#aaa",margin:"0 0 10px"}}>Costi + compenso obiettivo € {fmt(Number(obiettivoAnno.compensoTarget)*12)}/anno</p><div style={{display:"flex",justifyContent:"space-between",marginBottom:4}}><span style={{fontSize:13,color:"#666"}}>Target totale:</span><span style={{fontSize:13,fontWeight:500}}>€ {fmt(breakEvenObiettivo)}</span></div><div style={{display:"flex",justifyContent:"space-between",marginBottom:4}}><span style={{fontSize:13,color:"#666"}}>Quota netta incassata:</span><span style={{fontSize:13,fontWeight:500,color:STATI_CFG.Incassato.clr}}>€ {fmt(quotaNettoAgenzia)}</span></div><div style={S.progressBar(percObiettivo,BRAND.oro)}/><div style={{display:"flex",justifyContent:"space-between",marginTop:6}}><span style={{fontSize:12,color:"#aaa"}}>{percObiettivo}% raggiunto</span><span style={{fontSize:12,fontWeight:500,color:quotaNettoAgenzia>=breakEvenObiettivo?"#27AE60":BRAND.oroD}}>{quotaNettoAgenzia>=breakEvenObiettivo?`✅ Superato di € ${fmt(quotaNettoAgenzia-breakEvenObiettivo)}`:`Mancano € ${fmt(breakEvenObiettivo-quotaNettoAgenzia)}`}</span></div></div></div><div style={S.tblWrap}><div style={{padding:"10px 16px",background:"#fafaf8",borderBottom:"0.5px solid #eee",fontSize:13,fontWeight:500,color:"#666"}}>Andamento mensile {costiAnno}</div><table style={{...S.tbl,minWidth:400}}><thead><tr>{["Mese","Quota netta incassata","Spese consuntive","Saldo mese","Cumulato quota netta"].map(h=><th key={h} style={S.th}>{h}</th>)}</tr></thead><tbody>{Array.from({length:12},(_,i)=>{const m=`${costiAnno}-${String(i+1).padStart(2,"0")}`;const d=consuntiviMensili[m]||{incassato:0,spese:0,netto:0};const cumul=Array.from({length:i+1},(_,j)=>{const mk=`${costiAnno}-${String(j+1).padStart(2,"0")}`;return(consuntiviMensili[mk]||{incassato:0}).incassato;}).reduce((a,b)=>a+b,0);return(<tr key={m}><td style={S.td}>{MESI_NOMI[i+1]}</td><td style={S.tdR}>{d.incassato>0?<span style={{color:STATI_CFG.Incassato.clr,fontWeight:500}}>€ {fmt(d.incassato)}</span>:<span style={{color:"#ddd"}}>—</span>}</td><td style={S.tdR}>{d.spese>0?<span style={{color:"#E74C3C"}}>€ {fmt(d.spese)}</span>:<span style={{color:"#ddd"}}>—</span>}</td><td style={S.tdR}><span style={{color:d.netto>=0?"#27AE60":"#E74C3C",fontWeight:500}}>{d.incassato>0||d.spese>0?`€ ${fmt(d.netto)}`:"—"}</span></td><td style={S.tdR}>{cumul>0?<><span style={{fontWeight:500,color:cumul>=breakEvenBase?"#27AE60":BRAND.oroD}}>€ {fmt(cumul)}</span>{cumul>=breakEvenBase&&<span style={{fontSize:11,marginLeft:4,color:"#27AE60"}}>✅ BE</span>}</>:<span style={{color:"#ddd"}}>—</span>}</td></tr>);})}</tbody><tfoot><tr style={S.totRow}><td style={S.td}>Totale anno</td><td style={S.tdR}>€ {fmt(quotaNettoAgenzia)}</td><td style={S.tdR}>€ {fmt(totaleConsuntivo)}</td><td style={S.tdR}><strong>€ {fmt(quotaNettoAgenzia-totaleConsuntivo)}</strong></td><td style={S.td}/></tr></tfoot></table></div><div style={S.tblWrap}><div style={{padding:"10px 16px",background:"#fafaf8",borderBottom:"0.5px solid #eee",fontSize:13,fontWeight:500,color:"#666"}}>Voci di costo {costiAnno}</div><table style={{...S.tbl,minWidth:700}}><thead><tr>{["Voce","Tipo","Frequenza","Importo prev.","Totale annuo","Consuntivo","Azioni"].map(h=><th key={h} style={S.th}>{h}</th>)}</tr></thead><tbody>{costiAnnoCorrente.map(c=>{const annuo=c.frequenza==="Mensile"?Number(c.importoPrevisionale||0)*12:Number(c.importoPrevisionale||0);const cons=Object.values(c.consuntivi||{}).reduce((a,v)=>a+Number(v||0),0);return(<tr key={c.id}><td style={S.td}><strong>{c.voce}</strong></td><td style={S.td}><span style={{fontSize:11,padding:"2px 7px",borderRadius:4,background:c.tipo==="Fisso"?"#EAF4FB":"#FEF0E0",color:c.tipo==="Fisso"?"#2980B9":"#E67E22"}}>{c.tipo}</span></td><td style={S.td}>{c.frequenza}</td><td style={S.tdR}>€ {fmt(c.importoPrevisionale||0)}<span style={{fontSize:11,color:"#aaa"}}> /{c.frequenza==="Mensile"?"mese":"anno"}</span></td><td style={S.tdR}><strong>€ {fmt(annuo)}</strong></td><td style={S.tdR}>{cons>0?<span style={{color:"#E74C3C",fontWeight:500}}>€ {fmt(cons)}</span>:<span style={{color:"#ddd"}}>—</span>}</td><td style={S.td}><div style={{display:"flex",gap:4}}><button style={S.btnSm} onClick={()=>setShowConsuntivo(c)}>+ Cons.</button><button style={S.btnSm} onClick={()=>{setFormCosto({voce:c.voce,tipo:c.tipo,frequenza:c.frequenza,importoPrevisionale:c.importoPrevisionale});setEditCostoId(c.id);setShowFormCosto(true);}}>✎</button><button style={{...S.btnSm,color:"#c0392b"}} onClick={()=>eliminaCosto(c.id)}>✕</button></div></td></tr>);})}</tbody><tfoot><tr style={S.totRow}><td colSpan={4} style={S.td}>Totale</td><td style={S.tdR}><strong>€ {fmt(totalePrevisionale)}</strong></td><td style={S.tdR}>{totaleConsuntivo>0?<strong style={{color:"#E74C3C"}}>€ {fmt(totaleConsuntivo)}</strong>:"—"}</td><td style={S.td}/></tr></tfoot></table></div></div>)}

      {tab==="Agenti"&&(<div style={S.sec}><div style={{display:"flex",justifyContent:"flex-end",marginBottom:"1rem"}}><button style={S.btnP} onClick={()=>{setEditAgId(null);setFormAg(emptyAg());setShowAg(true);}}>+ Nuovo agente</button></div><div style={S.tblWrap}><table style={{...S.tbl,minWidth:500}}><thead><tr>{["Nome","Cognome","Profilo","Tipo","% Listing","% Acquirente","Azioni"].map(h=><th key={h} style={S.th}>{h}</th>)}</tr></thead><tbody>{agenti.map(a=>(<tr key={a.id}><td style={S.td}>{a.nome}</td><td style={S.td}>{a.cognome}</td><td style={S.td}>{a.profilo}</td><td style={S.td}>{a.tipo}</td><td style={S.td}>{a.profilo==="Broker"?"—":`${a.percListing}%`}</td><td style={S.td}>{a.profilo==="Broker"?"—":`${a.percAcquirente}%`}</td><td style={S.td}><div style={{display:"flex",gap:4}}><button style={S.btn} onClick={()=>apriEditAg(a)}>Modifica</button><button style={S.btnDanger} onClick={()=>setAgenti(agenti.filter(x=>x.id!==a.id))}>Elimina</button></div></td></tr>))}</tbody></table></div></div>)}

      {tab==="Impostazioni"&&(<div style={S.sec}><div style={{marginBottom:"1.5rem"}}><h2 style={S.sezTitle}>Tipi di vincolo</h2><div style={S.tagRow}>{vincoli.map(v=><span key={v} style={S.tag}>{v}<button style={S.tagX} onClick={()=>setVincoli(vincoli.filter(x=>x!==v))}>✕</button></span>)}</div><div style={{display:"flex",gap:8,maxWidth:360}}><input style={S.inp} placeholder="Nuovo tipo vincolo..." value={nuovoVincolo} onChange={e=>setNuovoVincolo(e.target.value)} onKeyDown={e=>{if(e.key==="Enter"&&nuovoVincolo.trim()){setVincoli([...vincoli,nuovoVincolo.trim()]);setNuovoVincolo("");}}}/><button style={S.btnP} onClick={()=>{if(nuovoVincolo.trim()){setVincoli([...vincoli,nuovoVincolo.trim()]);setNuovoVincolo("");}}}>Aggiungi</button></div></div><div style={{borderTop:"0.5px solid #eee",paddingTop:"1.25rem"}}><h2 style={S.sezTitle}>Backup dati</h2><p style={{fontSize:13,color:"#aaa",marginBottom:"0.75rem"}}>Esporta tutti i dati in JSON. Importa per ripristinarli.</p><div style={{display:"flex",gap:8}}><button style={S.btnP} onClick={esportaDati}>⬇ Esporta JSON</button><button style={S.btn} onClick={()=>importRef.current.click()}>⬆ Importa JSON</button></div></div></div>)}

      {showFormCosto&&(<div style={S.overlay} onClick={e=>e.target===e.currentTarget&&setShowFormCosto(false)}><div style={{...S.modal,width:"min(96vw,440px)"}}><h2 style={{fontSize:17,fontWeight:500,margin:"0 0 1rem"}}>{editCostoId?"Modifica voce":"Nuova voce di costo"}</h2><div style={{marginBottom:10}}><label style={S.lbl}>Voce di costo</label><input style={S.inp} value={formCosto.voce} onChange={e=>setFormCosto({...formCosto,voce:e.target.value})} placeholder="es. Affitto ufficio"/></div><div style={S.g2}><div><label style={S.lbl}>Tipo</label><select style={S.inp} value={formCosto.tipo} onChange={e=>setFormCosto({...formCosto,tipo:e.target.value})}><option>Fisso</option><option>Variabile</option></select></div><div><label style={S.lbl}>Frequenza</label><select style={S.inp} value={formCosto.frequenza} onChange={e=>setFormCosto({...formCosto,frequenza:e.target.value})}><option>Mensile</option><option>Annuale</option></select></div></div><div style={{marginBottom:10}}><label style={S.lbl}>Importo previsionale (€) {formCosto.frequenza==="Mensile"?"al mese":"all'anno"}</label><input style={S.inp} type="number" value={formCosto.importoPrevisionale} onChange={e=>setFormCosto({...formCosto,importoPrevisionale:Number(e.target.value)})}/></div>{formCosto.frequenza==="Mensile"&&Number(formCosto.importoPrevisionale)>0&&<p style={{fontSize:12,color:"#aaa",margin:"0 0 10px"}}>Totale annuo: <strong>€ {fmt(Number(formCosto.importoPrevisionale)*12)}</strong></p>}<div style={{display:"flex",gap:8,justifyContent:"flex-end",marginTop:"1rem"}}><button style={S.btn} onClick={()=>{setShowFormCosto(false);setEditCostoId(null);}}>Annulla</button><button style={S.btnP} onClick={salvaCosto}>Salva</button></div></div></div>)}

      {showConsuntivo&&(<div style={S.overlay} onClick={e=>e.target===e.currentTarget&&setShowConsuntivo(null)}><div style={{...S.modal,width:"min(96vw,420px)"}}><h2 style={{fontSize:17,fontWeight:500,margin:"0 0 4px"}}>Inserisci consuntivo</h2><p style={{fontSize:13,color:"#aaa",margin:"0 0 1rem"}}>{showConsuntivo.voce}</p><div style={S.g2}><div><label style={S.lbl}>Mese</label><select style={S.inp} value={consuntivoMese} onChange={e=>setConsuntivoMese(e.target.value)}><option value="">Seleziona mese</option>{Array.from({length:12},(_,i)=>{const m=`${costiAnno}-${String(i+1).padStart(2,"0")}`;return <option key={m} value={m}>{MESI_NOMI[i+1]}</option>;})}</select></div><div><label style={S.lbl}>Importo effettivo (€)</label><input style={S.inp} type="number" value={consuntivoImporto} onChange={e=>setConsuntivoImporto(e.target.value)}/></div></div>{consuntivoMese&&showConsuntivo.consuntivi?.[consuntivoMese]&&<p style={{fontSize:12,color:"#aaa"}}>Già inserito: € {fmt(showConsuntivo.consuntivi[consuntivoMese])}</p>}<div style={{marginTop:"1rem"}}><p style={{fontSize:12,color:"#aaa",marginBottom:6}}>Consuntivi inseriti:</p>{Object.entries(showConsuntivo.consuntivi||{}).filter(([,v])=>Number(v)>0).map(([k,v])=><div key={k} style={{display:"flex",justifyContent:"space-between",fontSize:13,padding:"3px 0",borderBottom:"0.5px solid #f0f0f0"}}><span>{fmtMese(k)}</span><span style={{color:"#E74C3C",fontWeight:500}}>€ {fmt(v)}</span></div>)}</div><div style={{display:"flex",gap:8,justifyContent:"flex-end",marginTop:"1rem"}}><button style={S.btn} onClick={()=>setShowConsuntivo(null)}>Chiudi</button><button style={S.btnP} onClick={salvaConsuntivo}>Salva</button></div></div></div>)}

      {showAvanza&&(<div style={S.overlay} onClick={e=>e.target===e.currentTarget&&setShowAvanza(null)}><div style={S.modal}><h2 style={{fontSize:17,fontWeight:500,margin:"0 0 4px"}}>Avanza stato pratica</h2><p style={{fontSize:13,color:"#aaa",margin:"0 0 1rem"}}>{showAvanza.p.comune} — {showAvanza.p.indirizzo} · Prezzo offerto: <strong>€ {fmtN(showAvanza.p.prezzoVendita)}</strong></p><div style={{marginBottom:12}}><label style={S.lbl}>Nuovo stato</label><div style={{display:"flex",gap:8,flexWrap:"wrap"}}>{showAvanza.next.map(s=>(<button key={s} onClick={()=>setAvForm({...avForm,stato:s})} style={{...S.btn,border:`1.5px solid ${avForm.stato===s?STATI_CFG[s]?.clr:"#ddd"}`,background:avForm.stato===s?STATI_CFG[s]?.bg:"#fff",color:avForm.stato===s?STATI_CFG[s]?.clr:BRAND.grigio,fontWeight:avForm.stato===s?500:400}}>{STATI_CFG[s]?.semaforo} {s}</button>))}</div></div>{["Rifiutata","Mancata Chiusura","Controproposta"].includes(avForm.stato)&&(<div style={S.warnBox}><label style={S.lbl}>{avForm.stato==="Controproposta"?"Note controproposta":avForm.stato==="Mancata Chiusura"?"Motivo mancata chiusura":"Motivo rifiuto"}</label><textarea style={{...S.inp,resize:"vertical",minHeight:64}} value={avForm.noteStato||""} onChange={e=>setAvForm({...avForm,noteStato:e.target.value})}/></div>)}{avForm.stato==="Accettata con Vincolo"&&(<div style={S.warnBox}><p style={{fontSize:13,fontWeight:500,margin:"0 0 10px",color:"#D4AC0D"}}>Dettagli vincolo</p><div style={S.g2}><div><label style={S.lbl}>Tipo vincolo</label><select style={S.inp} value={avForm.tipoVincolo||""} onChange={e=>setAvForm({...avForm,tipoVincolo:e.target.value})}><option value="">Seleziona</option>{vincoli.map(v=><option key={v}>{v}</option>)}</select></div><div><label style={S.lbl}>Scadenza</label><input style={S.inp} type="date" value={avForm.dataVincolo||""} onChange={e=>setAvForm({...avForm,dataVincolo:e.target.value})}/></div></div><div style={{display:"flex",alignItems:"center",gap:8,marginTop:8}}><input type="checkbox" id="esito" checked={avForm.esitoVincolo} onChange={e=>setAvForm({...avForm,esitoVincolo:e.target.checked})}/><label htmlFor="esito" style={{fontSize:13}}>Esito positivo → diventa <strong>Accettata</strong></label></div></div>)}{avForm.stato==="Accettata"&&<ConfermaProvv/>}{["Preliminare","Rogito Diretto"].includes(avForm.stato)&&(<><ConfermaProvv/><div style={{...S.confBox,background:"#EAF4FB",borderColor:"#2980B9"}}><p style={{fontSize:13,fontWeight:500,margin:"0 0 10px",color:"#2980B9"}}>Dettagli atto</p><div style={S.g2}><div><label style={S.lbl}>Data atto</label><input style={S.inp} type="date" value={avForm.dataAtto||""} onChange={e=>setAvForm({...avForm,dataAtto:e.target.value})}/></div><div><label style={S.lbl}>Acconto provvigioni (€)</label><input style={S.inp} type="number" value={avForm.accontoProvv||0} onChange={e=>setAvForm({...avForm,accontoProvv:e.target.value})}/></div></div></div></>)}{avForm.stato==="Incassato"&&(<div style={{marginBottom:10}}><label style={S.lbl}>Note</label><textarea style={{...S.inp,resize:"vertical",minHeight:48}} value={avForm.noteStato||""} onChange={e=>setAvForm({...avForm,noteStato:e.target.value})}/></div>)}<div style={{display:"flex",gap:8,justifyContent:"flex-end",marginTop:"1rem"}}><button style={S.btn} onClick={()=>setShowAvanza(null)}>Annulla</button><button style={S.btnP} onClick={salvaAvanza}>Conferma</button></div></div></div>)}

      {showProp&&(<div style={S.overlay} onClick={e=>e.target===e.currentTarget&&setShowProp(false)}><div style={S.modal}><h2 style={{fontSize:17,fontWeight:500,margin:"0 0 1rem"}}>{editPropId?"Modifica proposta":"Nuova proposta"}</h2><div style={S.g2}><div><label style={S.lbl}>Data proposta (aaaa-mm)</label><input style={S.inp} value={form.dataProposta} onChange={e=>hFormChange("dataProposta",e.target.value)} placeholder="2026-04"/></div><div><label style={S.lbl}>Stato iniziale</label><select style={S.inp} value={form.stato} onChange={e=>hFormChange("stato",e.target.value)}>{Object.keys(STATI_CFG).map(s=><option key={s}>{s}</option>)}</select></div><div><label style={S.lbl}>Comune</label><input style={S.inp} value={form.comune} onChange={e=>hFormChange("comune",e.target.value)}/></div><div><label style={S.lbl}>Indirizzo</label><input style={S.inp} value={form.indirizzo} onChange={e=>hFormChange("indirizzo",e.target.value)}/></div><div><label style={S.lbl}>Tipologia</label><select style={S.inp} value={form.tipologia} onChange={e=>hFormChange("tipologia",e.target.value)}><option value="">Seleziona</option>{TIPOLOGIE.map(t=><option key={t}>{t}</option>)}</select></div><div><label style={S.lbl}>Prezzo offerto (€)</label><input style={S.inp} type="number" value={form.prezzoVendita} onChange={e=>hFormChange("prezzoVendita",e.target.value)}/></div><div><label style={S.lbl}>Cliente venditore</label><input style={S.inp} value={form.venditore} onChange={e=>hFormChange("venditore",e.target.value)}/></div><div><label style={S.lbl}>Cliente acquirente</label><input style={S.inp} value={form.acquirente} onChange={e=>hFormChange("acquirente",e.target.value)}/></div></div><p style={{fontSize:13,fontWeight:500,margin:"8px 0 6px",borderTop:"0.5px solid #eee",paddingTop:10}}>Lato venditore</p><div style={S.g2}><div><label style={S.lbl}>Agente listing</label><select style={S.inp} value={form.agenteListing} onChange={e=>hFormChange("agenteListing",e.target.value)}><option value="">Seleziona</option>{agenti.map(a=><option key={a.id} value={a.id}>{a.nome} {a.cognome} ({a.profilo})</option>)}</select></div><div><label style={S.lbl}>% Listing pattuita</label><input style={S.inp} type="number" step="0.1" value={form.percListing} onChange={e=>hFormChange("percListing",e.target.value)}/></div><div><label style={S.lbl}>Buyer L (opzionale)</label><select style={S.inp} value={form.buyerL} onChange={e=>hFormChange("buyerL",e.target.value)}><option value="">Nessuno</option>{agenti.map(a=><option key={a.id} value={a.id}>{a.nome} {a.cognome}</option>)}</select></div><div><label style={S.lbl}>% Buyer L</label><input style={S.inp} type="number" step="0.1" value={form.percBuyerL} onChange={e=>hFormChange("percBuyerL",e.target.value)}/></div><div><label style={S.lbl}>Provv. venditore stimata (€)</label><input style={S.inp} type="number" value={form.provvVenditore} onChange={e=>hFormChange("provvVenditore",e.target.value)}/></div></div><p style={{fontSize:13,fontWeight:500,margin:"8px 0 6px",borderTop:"0.5px solid #eee",paddingTop:10}}>Lato acquirente</p><div style={S.g2}><div><label style={S.lbl}>Agente acquirente</label><select style={S.inp} value={form.agenteAcquirente} onChange={e=>hFormChange("agenteAcquirente",e.target.value)}><option value="">Seleziona</option>{agenti.map(a=><option key={a.id} value={a.id}>{a.nome} {a.cognome} ({a.profilo})</option>)}</select></div><div><label style={S.lbl}>% Acquirente pattuita</label><input style={S.inp} type="number" step="0.1" value={form.percAcquirente} onChange={e=>hFormChange("percAcquirente",e.target.value)}/></div><div><label style={S.lbl}>Buyer (opzionale)</label><select style={S.inp} value={form.buyer} onChange={e=>hFormChange("buyer",e.target.value)}><option value="">Nessuno</option>{agenti.map(a=><option key={a.id} value={a.id}>{a.nome} {a.cognome}</option>)}</select></div><div><label style={S.lbl}>% Buyer</label><input style={S.inp} type="number" step="0.1" value={form.percBuyer} onChange={e=>hFormChange("percBuyer",e.target.value)}/></div><div><label style={S.lbl}>Provv. acquirente stimata (€)</label><input style={S.inp} type="number" value={form.provvAcquirente} onChange={e=>hFormChange("provvAcquirente",e.target.value)}/></div></div><div style={{marginTop:8}}><label style={S.lbl}>Note</label><textarea style={{...S.inp,resize:"vertical",minHeight:48}} value={form.noteStato||""} onChange={e=>hFormChange("noteStato",e.target.value)}/></div><div style={{display:"flex",gap:8,justifyContent:"flex-end",marginTop:"1rem"}}><button style={S.btn} onClick={()=>{setShowProp(false);setEditPropId(null);}}>Annulla</button><button style={S.btnP} onClick={salvaProposta}>Salva</button></div></div></div>)}

      {showAg&&(<div style={S.overlay} onClick={e=>e.target===e.currentTarget&&setShowAg(false)}><div style={{...S.modal,width:"min(96vw,420px)"}}><h2 style={{fontSize:17,fontWeight:500,margin:"0 0 1rem"}}>{editAgId?"Modifica agente":"Nuovo agente"}</h2><div style={S.g2}><div><label style={S.lbl}>Nome</label><input style={S.inp} value={formAg.nome} onChange={e=>setFormAg({...formAg,nome:e.target.value})}/></div><div><label style={S.lbl}>Cognome</label><input style={S.inp} value={formAg.cognome} onChange={e=>setFormAg({...formAg,cognome:e.target.value})}/></div><div><label style={S.lbl}>Profilo</label><select style={S.inp} value={formAg.profilo} onChange={e=>setFormAg({...formAg,profilo:e.target.value})}>{PROFILI.map(p=><option key={p}>{p}</option>)}</select></div><div><label style={S.lbl}>Tipo</label><select style={S.inp} value={formAg.tipo} onChange={e=>setFormAg({...formAg,tipo:e.target.value})}><option>Interno</option><option>Agenzia esterna</option></select></div>{formAg.profilo!=="Broker"&&<><div><label style={S.lbl}>% default Listing</label><input style={S.inp} type="number" step="0.1" value={formAg.percListing} onChange={e=>setFormAg({...formAg,percListing:Number(e.target.value)})}/></div><div><label style={S.lbl}>% default Acquirente</label><input style={S.inp} type="number" step="0.1" value={formAg.percAcquirente} onChange={e=>setFormAg({...formAg,percAcquirente:Number(e.target.value)})}/></div></>}</div><div style={{display:"flex",gap:8,justifyContent:"flex-end",marginTop:"1rem"}}><button style={S.btn} onClick={()=>{setShowAg(false);setEditAgId(null);}}>Annulla</button><button style={S.btnP} onClick={salvaAgente}>Salva</button></div></div></div>)}

      {showStorico&&(<div style={S.overlay} onClick={e=>e.target===e.currentTarget&&setShowStorico(null)}><div style={{...S.modal,width:"min(96vw,420px)"}}><h2 style={{fontSize:17,fontWeight:500,margin:"0 0 4px"}}>Storico stati</h2><p style={{fontSize:13,color:"#aaa",margin:"0 0 1.25rem"}}>{showStorico.comune} — {showStorico.indirizzo}</p>{(showStorico.storico||[]).map((s,i,arr)=>{const l=i===arr.length-1;return(<div key={i} style={S.storicoStep(l)}><div style={S.dot(l)}/><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",gap:8}}><span style={badgeStyle(s.stato)}>{STATI_CFG[s.stato]?.semaforo} {s.stato}</span><span style={{fontSize:11,color:"#aaa"}}>{fmtDate(s.data)}</span></div>{s.note&&<p style={{fontSize:12,color:"#aaa",margin:"4px 0 0",fontStyle:"italic"}}>{s.note}</p>}</div>);})}<div style={{display:"flex",justifyContent:"flex-end",marginTop:"1rem"}}><button style={S.btn} onClick={()=>setShowStorico(null)}>Chiudi</button></div></div></div>)}
    </div>
  );
}
