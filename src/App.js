...
const TAB_CONFIG = [
  ...
  { id:"Fattura Agente",icon:"🧾",label:"Fattura Agente" },
...
const STATI_INCASSO = {
  "Da incassare":{clr:"#E67E22",bg:"#FEF0E0"},
  "Parziale":{clr:"#D4AC0D",bg:"#FEF9E7"},
  "Incassato":{clr:"#27AE60",bg:"#E9F7EF"},
};
...
const calcolaStatoIncasso = v => {
  const totP = Number(v.provvVenditore||0)+Number(v.provvAcquirente||0);
  const totI = calcolaIncassatoV(v)+calcolaIncassatoA(v);
  if(totI===0) return "Da incassare";
  if(totI>=totP) return "Incassato";
  return "Parziale";
};

const VOCI_COSTO = ["Locazione Ufficio","Spese Condominiali","Utenza Elettricità","Utenza GAS","Telefonia Fissa","Telefonia Cellulare","Pulizie","Assicurazione Ufficio","Imposte Pubblicitarie","Ufficio Multifunzione Canone","Commercialista SRL","Consulente Paghe","Compenso Amministratore","Stipendio Erica Guglielmana","Stipendi x collaborazioni","Tasse / Contributi x Dipendenti","Immobiliare.it","Idealista.it & Casa.it","Sponsorizzazioni Squadre","Gestim + Sito e Hosting","Software - Servizi Professionali","FIAIP","Assicurazioni Professionali","Altre Assicurazioni","Agente Strategico Abbonamento"];
const mkCosti = () => VOCI_COSTO.map((v,i)=>({id:i+1,voce:v,tipo:"Fisso",frequenza:"Mensile",importoPrevisionale:0,consuntivi:{}}));
...
function Sidebar({tab, setTab, utente, onEsporta, onImporta, importRef}) {
  return (
    <div style={{
      width:220, minWidth:220, background:"#2C2C2C", display:"flex", flexDirection:"column",
      height:"100vh", position:"sticky", top:0, boxShadow:"2px 0 12px rgba(0,0,0,0.15)", flexShrink:0
    }}>
      {/* Logo area */}
      <div style={{padding:"1.5rem 1.25rem 1.25rem", borderBottom:"1px solid rgba(255,255,255,0.08)"}}>
        <div style={{lineHeight:1}}>
          <div style={{fontSize:28,fontWeight:700,color:"#fff",fontFamily:"Georgia,serif",letterSpacing:"-0.5px"}}>
            c<span style={{color:BRAND.oro}}>à</span>sa
          </div>
          <div style={{fontSize:8,letterSpacing:"0.3em",color:"rgba(255,255,255,0.4)",borderTop:`1px solid rgba(255,255,255,0.2)`,paddingTop:3,marginTop:3}}>
            IMMOBILIARE
          </div>
        </div>
        <div style={{marginTop:10,fontSize:11,color:"rgba(255,255,255,0.35)",letterSpacing:"0.02em"}}>
          Gestionale interno
        </div>
      </div>
        ...
        {TAB_CONFIG.map(t => {
          ...
          return (
            <button key={t.id} onClick={()=>setTab(t.id)} style={{
              ...
              background: active ? `${BRAND.oro}22` : "transparent",
              border:"none", borderLeft: active ? `3px solid ${BRAND.oro}` : "3px solid transparent",
              color: active ? BRAND.oro : "rgba(255,255,255,0.55)",
            ...
            >
              <span style={{fontSize:15, lineHeight:1, width:18, textAlign:"center", flexShrink:0}}>{t.icon}</span>
              <span>{t.label}</span>
            </button>
          );
        })}
      ...
      <div style={{padding:"0.75rem 1rem"}}>
        <button onClick={onEsporta} style={{display:"flex",alignItems:"center",gap:8,width:"100%",padding:"8px 10px",background:"transparent",border:"1px solid rgba(201,169,110,0.4)",borderRadius:6,color:BRAND.oro,fontSize:12,cursor:"pointer",marginBottom:6}}>
          <span>⬇</span> Esporta dati
        </button>
        <button onClick={()=>importRef.current.click()} style={{display:"flex",alignItems:"center",gap:8,width:"100%",padding:"8px 10px",background:"transparent",border:"1px solid rgba(255,255,255,0.12)",borderRadius:6,color:"rgba(255,255,255,0.45)",fontSize:12,cursor:"pointer"}}>
      ...
      <div style={{padding:"1rem 1.25rem", borderTop:"1px solid rgba(255,255,255,0.08)", background:"rgba(0,0,0,0.2)"}}>
        <div style={{display:"flex",alignItems:"center",gap:8}}>
          <div style={{width:30,height:30,borderRadius:"50%",background:`linear-gradient(135deg,${BRAND.oro},#A8863A)`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:13,fontWeight:700,color:"#fff",flexShrink:0}}>
...
function ModalIncassoLato({vend, lato, onSave, onClose}) {
  const isV = lato === "V";
  const provv = isV ? Number(vend.provvVenditore||0) : Number(vend.provvAcquirente||0);
  const nominativo = isV ? vend.nominativoVenditore : vend.nomeAcquirente;

  const [form, setForm] = useState({
    acc1: Number(vend[`acc1${lato}`]||0), dataAcc1: vend[`dataAcc1${lato}`]||"", noteAcc1: vend[`noteAcc1${lato}`]||"",
    acc2: Number(vend[`acc2${lato}`]||0), dataAcc2: vend[`dataAcc2${lato}`]||"", noteAcc2: vend[`noteAcc2${lato}`]||"",
    saldo: Number(vend[`saldo${lato}`]||0), dataSaldo: vend[`dataSaldo${lato}`]||"", noteSaldo: vend[`noteSaldo${lato}`]||"",
    scadenzaIncasso: vend.scadenzaIncasso||"",
  });

  const totIncassato = Number(form.acc1||0)+Number(form.acc2||0)+Number(form.saldo||0);
  const residuo = provv - totIncassato;
  ...
  const S = {
    ...
    secTitle:{fontSize:12,fontWeight:600,color:BRAND.oroD,textTransform:"uppercase",letterSpacing:"0.08em",margin:"0 0 8px"},
    secBox:{background:BRAND.beige,borderRadius:8,padding:"12px 14px",marginBottom:10},
    row3:{display:"grid",gridTemplateColumns:"120px 1fr 1fr",gap:8},
  };

  const handleSave = () => {
    const updated = {
      ...vend,
      [`acc1${lato}`]: Number(form.acc1||0), [`dataAcc1${lato}`]: form.dataAcc1, [`noteAcc1${lato}`]: form.noteAcc1,
      [`acc2${lato}`]: Number(form.acc2||0), [`dataAcc2${lato}`]: form.dataAcc2, [`noteAcc2${lato}`]: form.noteAcc2,
      [`saldo${lato}`]: Number(form.saldo||0), [`dataSaldo${lato}`]: form.dataSaldo, [`noteSaldo${lato}`]: form.noteSaldo,
      scadenzaIncasso: form.scadenzaIncasso,
    };
    updated.incassatoVenditore = calcolaIncassatoV(updated);
    updated.incassatoAcquirente = calcolaIncassatoA(updated);
    updated.statoIncasso = calcolaStatoIncasso(updated);
    onSave(updated);
  };
  ...
  return (
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.45)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:300}}>
      <div style={{background:"#fff",borderRadius:12,border:"0.5px solid #ddd",padding:"1.5rem",width:"min(96vw,560px)",maxHeight:"90vh",overflowY:"auto"}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:"1rem"}}>
          <div>
            <h2 style={{fontSize:16,fontWeight:600,margin:"0 0 2px",color:BRAND.grigio}}>Incasso {isV?"Venditore":"Acquirente"}</h2>
            <p style={{fontSize:13,color:"#aaa",margin:0}}>{vend.comuneImmobile} — {vend.indirizzoImmobile} &nbsp;|&nbsp; <strong>{nominativo}</strong></p>
          </div>
          <button onClick={onClose} style={{background:"none",border:"none",fontSize:20,cursor:"pointer",color:"#ccc",lineHeight:1,padding:0}}>✕</button>
        </div>
        <div style={{background:`${BRAND.oro}18`,border:`1px solid ${BRAND.oro}44`,borderRadius:8,padding:"10px 14px",marginBottom:"1rem",display:"flex",justifyContent:"space-between",flexWrap:"wrap",gap:8}}>
          <span style={{fontSize:13,color:BRAND.grigio}}>Provvigione: <strong style={{color:BRAND.oroD}}>€ {fmt(provv)}</strong></span>
          <span style={{fontSize:13,fontWeight:600,color:residuo>0?"#E67E22":residuo===0?"#27AE60":"#E74C3C"}}>
            {residuo>0?`Residuo: € ${fmt(residuo)}`:residuo===0?"✅ Saldato":"⚠️ Eccesso: € "+fmt(-residuo)}
          </span>
        </div>
        {[["Acconto 1","acc1","dataAcc1","noteAcc1",S.secBox],["Acconto 2","acc2","dataAcc2","noteAcc2",S.secBox],["Saldo","saldo","dataSaldo","noteSaldo",{...S.secBox,background:"#E9F7EF",border:"1px solid #27AE6033"}]].map(([title,fA,fD,fN,boxStyle])=>(
          <div key={title} style={boxStyle}>
            <p style={{...S.secTitle,color:title==="Saldo"?"#27AE60":BRAND.oroD}}>{title}</p>
            <div style={S.row3}>
              <div><label style={S.lbl}>Importo (€)</label><input style={S.inp} type="number" value={form[fA]||""} onChange={e=>setForm({...form,[fA]:e.target.value})} placeholder="0"/></div>
              <div><label style={S.lbl}>Data</label><input style={S.inp} type="date" value={form[fD]} onChange={e=>setForm({...form,[fD]:e.target.value})}/></div>
              <div><label style={S.lbl}>Nota</label><input style={S.inp} value={form[fN]} onChange={e=>setForm({...form,[fN]:e.target.value})}/></div>
            </div>
          </div>
        ))}
        <div style={{marginBottom:"1rem"}}>
          <label style={S.lbl}>Scadenza incasso prevista</label>
          <input style={{...S.inp,maxWidth:200}} type="date" value={form.scadenzaIncasso} onChange={e=>setForm({...form,scadenzaIncasso:e.target.value})}/>
        </div>
        <div style={{background:residuo>0?"#FEF0E0":residuo===0?"#E9F7EF":"#FDECEA",borderRadius:8,padding:"10px 14px",marginBottom:"1rem",display:"flex",gap:16,flexWrap:"wrap",fontSize:13}}>
          {Number(form.acc1)>0&&<span>Acc.1: <strong>€ {fmt(form.acc1)}</strong></span>}
          {Number(form.acc2)>0&&<span>Acc.2: <strong>€ {fmt(form.acc2)}</strong></span>}
          {Number(form.saldo)>0&&<span>Saldo: <strong>€ {fmt(form.saldo)}</strong></span>}
          <span style={{marginLeft:"auto",fontWeight:600,color:residuo>0?"#E67E22":residuo===0?"#27AE60":"#E74C3C"}}>Totale: € {fmt(totIncassato)} / € {fmt(provv)}</span>
        </div>
        <div style={{display:"flex",gap:8,justifyContent:"flex-end"}}>
          <button onClick={onClose} style={{padding:"7px 14px",fontSize:13,borderRadius:6,border:"0.5px solid #ccc",background:"#fff",cursor:"pointer",color:BRAND.grigio}}>Annulla</button>
          <button onClick={handleSave} style={{padding:"7px 14px",fontSize:13,borderRadius:6,border:`1px solid ${BRAND.oro}`,background:BRAND.oro,cursor:"pointer",color:"#fff",fontWeight:500}}>Salva</button>
        </div>
      </div>
    </div>
  );
...
export default function App() {
  ...
  const [costi,setCosti]=useState({[annoCorrente]:mkCosti()});
  const [obiettivi,setObiettivi]=useState({[annoCorrente]:{fatturato:360000,compensoTarget:5000}});
  ...
  const [showAgente,setShowAgente]=useState(null); // null | "new" | agente obj
  ...
  const incFiltrati=useMemo(()=>incarichi.filter(i=>{
    ...
    return true;
  }),[incarichi,subInc,fIncStato,fIncAnno,fIncMese,fIncAg]);

  const cntInc=useMemo(()=>{
    const base=incarichi.filter(i=>{
      if(i.categoria!==subInc) return false;
      if(fIncAnno!=="Tutti"&&getAnno(i.dataInizio)!==fIncAnno) return false;
      if(fIncMese!=="Tutti"&&getMese(i.dataInizio)!==fIncMese) return false;
      if(fIncAg!=="Tutti"&&i.agenteListing!==Number(fIncAg)) return false;
    ...
    return {attivi:base.filter(i=>statoInc(i)==="Attivo").length,scaduti:base.filter(i=>statoInc(i)==="Scaduto").length,venduti:base.filter(i=>statoInc(i)==="Venduto"||statoInc(i)==="Locato").length};
  ...
  const cntProp=useMemo(()=>({
    attesa:propFiltrate.filter(p=>p.stato==="In attesa").length,
    vincolo:propFiltrate.filter(p=>p.stato==="Accettata con Vincolo").length,
    accettate:propFiltrate.filter(p=>p.stato==="Accettata").length,
    rifiutate:propFiltrate.filter(p=>["Rifiutata","Mancata Chiusura"].includes(p.stato)).length,
  }),[propFiltrate]);

  const vendFiltrati=useMemo(()=>venduti.filter(v=>{
    if(v.categoria!==subVend) return false;
    if(fVendStato!=="Tutti"&&v.statoIncasso!==fVendStato) return false;
  ...
  const cntVend=useMemo(()=>({
    daIncassare:vendFiltrati.filter(v=>v.statoIncasso==="Da incassare").length,
    parziale:vendFiltrati.filter(v=>v.statoIncasso==="Parziale").length,
    incassato:vendFiltrati.filter(v=>v.statoIncasso==="Incassato").length,
  }),[vendFiltrati]);
  ...
  const totProvvDash=dashVend.reduce((s,v)=>s+Number(v.provvVenditore||0)+Number(v.provvAcquirente||0),0);
  const totIncDash=dashVend.reduce((s,v)=>s+calcolaIncassatoV(v)+calcolaIncassatoA(v),0);

  const sospesi=useMemo(()=>{
    const rows=[];
    venduti.filter(v=>v.categoria==="vendita"&&v.statoIncasso!=="Incassato"&&(dashAnno==="Tutti"||getAnno(v.dataAtto||"")===dashAnno)).forEach(v=>{
      const dV=Number(v.provvVenditore||0)-calcolaIncassatoV(v);
      const dA=Number(v.provvAcquirente||0)-calcolaIncassatoA(v);
      if(dV>0) rows.push({vendId:v.id,lato:"V",agente:nomAg(v.agenteListing),nominativo:v.nominativoVenditore,immobile:`${v.comuneImmobile} - ${v.indirizzoImmobile}`,importo:dV,scadenza:v.scadenzaIncasso||"",acc1:v.acc1V||0,dataAcc1:v.dataAcc1V||"",acc2:v.acc2V||0,dataAcc2:v.dataAcc2V||"",saldo:v.saldoV||0,dataSaldo:v.dataSaldoV||"",totaleProvv:Number(v.provvVenditore||0)});
      if(dA>0) rows.push({vendId:v.id,lato:"A",agente:nomAg(v.agenteAcquirente),nominativo:v.nomeAcquirente,immobile:`${v.comuneImmobile} - ${v.indirizzoImmobile}`,importo:dA,scadenza:v.scadenzaIncasso||"",acc1:v.acc1A||0,dataAcc1:v.dataAcc1A||"",acc2:v.acc2A||0,dataAcc2:v.dataAcc2A||"",saldo:v.saldoA||0,dataSaldo:v.dataSaldoA||"",totaleProvv:Number(v.provvAcquirente||0)});
    });
    return rows;
  },[venduti,dashAnno,agenti]);

  const handleSaveIncassoLato = (updatedVend) => {
    setVenduti(venduti.map(v=>v.id===updatedVend.id?updatedVend:v));
    setShowIncassoLato(null);
  };
  ...
  const fatturaDati=useMemo(()=>{
    ...
    return venduti.filter(v=>{
      if(v.statoIncasso==="Da incassare") return false;
      if(fatAnno!=="Tutti"&&getAnno(v.dataAtto||"")!==fatAnno) return false;
      if(fatMese!=="Tutti"&&getMese(v.dataAtto||"")!==fatMese) return false;
      ...
      return {v,righe,totPratica:righe.reduce((s,r)=>s+r.quotaAg,0)};
    }).filter(x=>x.righe.length>0);
  },[agenti,venduti,fatAgente,fatAnno,fatMese]);
  ...
  const salvaProp=()=>{
    ...
    const p={...formProp,id:Date.now(),prezzoOfferto:Number(formProp.prezzoOfferto),provvAcquirente:Number(formProp.provvAcquirente),provvVenditore:Number(formProp.provvVenditore),agenteAcquirente:Number(formProp.agenteAcquirente)||null,buyer:formProp.buyer?Number(formProp.buyer):null,percAcquirente:Number(formProp.percAcquirente||0)};
    setProposte([...proposte,p]);
    setShowProp(null);
  };

  const salvaStatoProp=()=>{
    if(!showGestProp) return;
    const p=showGestProp;
    const ns=formStatoProp.stato||p.stato;
    ...
    if(ns==="Accettata"||(ns==="Accettata con Vincolo"&&formStatoProp.esitoVincolo==="Positivo")){
      ...
      const ag=agenti.find(a=>a.id===p.agenteAcquirente);
  ...
  const salvaVend=()=>{
    if(!showGestVend) return;
    const updated={...showGestVend,...formVend,incassatoVenditore:calcolaIncassatoV(formVend),incassatoAcquirente:calcolaIncassatoA(formVend)};
    updated.statoIncasso=calcolaStatoIncasso(updated);
    setVenduti(venduti.map(v=>v.id===showGestVend.id?updated:v));
    setShowGestVend(null);
  };

  const esporta=()=>{const b=new Blob([JSON.stringify({agenti,incarichi,proposte,venduti,costi,obiettivi,fonti,tipologie,vincoli,tipiNeg},null,2)],{type:"application/json"});const u=URL.createObjectURL(b);const a=document.createElement("a");a.href=u;a.download=`gestionale_${todayStr()}.json`;a.click();URL.revokeObjectURL(u);};
  const importa=e=>{const f=e.target.files[0];if(!f) return;const r=new FileReader();r.onload=ev=>{try{const d=JSON.parse(ev.target.result);if(d.agenti)setAgenti(d.agenti);if(d.incarichi)setIncarichi(d.incarichi);if(d.proposte)setProposte(d.proposte);if(d.venduti)setVenduti(d.venduti);if(d.fonti)setFonti(d.fonti);if(d.tipologie)setTipologie(d.tipologie);if(d.vincoli)setVincoli(d.vincoli);if(d.tipiNeg)setTipiNeg(d.tipiNeg);alert("Importato!");}catch{alert("File non valido.");}};r.readAsText(f);e.target.value="";};
  ...
  const S={
    ...
    pageTitle:{fontSize:20,fontWeight:600,color:BRAND.grigio,margin:"0 0 1.25rem",fontFamily:"Georgia,serif"},
  ...
  const miniIncasso=(r)=>{
    const p=[];
    if(Number(r.acc1)>0) p.push(`Acc.1: €${fmtN(r.acc1)}${r.dataAcc1?" ("+fmtD(r.dataAcc1)+")":""}`);
    if(Number(r.acc2)>0) p.push(`Acc.2: €${fmtN(r.acc2)}${r.dataAcc2?" ("+fmtD(r.dataAcc2)+")":""}`);
    if(Number(r.saldo)>0) p.push(`Saldo: €${fmtN(r.saldo)}${r.dataSaldo?" ("+fmtD(r.dataSaldo)+")":""}`);
    return p.length>0?p.join(" · "):"—";
  };

  // Current tab label for page title
  ...
  return (
    <div style={{display:"flex",height:"100vh",background:BRAND.beige,overflow:"hidden",fontFamily:"'Georgia',serif",color:BRAND.grigio}}>
      ...
      <div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden"}}>
        ...
        {/* Top bar */}
        <div style={{background:"#fff",borderBottom:"0.5px solid #e8e5e0",padding:"0.875rem 1.5rem",display:"flex",alignItems:"center",justifyContent:"space-between",boxShadow:"0 1px 4px rgba(0,0,0,0.04)",flexShrink:0}}>
          <div style={{display:"flex",alignItems:"center",gap:8}}>
            <span style={{fontSize:18}}>{currentTabCfg?.icon}</span>
            <h1 style={{fontSize:15,fontWeight:600,margin:0,color:BRAND.grigio}}>{currentTabCfg?.label}</h1>
          </div>
          <button style={{...S.btn,color:"#c0392b",fontSize:12}} onClick={()=>setUtente(null)}>
            Esci
          </button>
        </div>
        ...
        <div style={{flex:1,overflowY:"auto"}}>
          ...
          {tab==="Dashboard"&&(<div style={S.sec}>
            ...
            <div style={S.g3}>
              <div style={S.card(BRAND.oro)}><p style={{fontSize:12,color:"#888",margin:"0 0 4px"}}>Totale provvigioni</p><p style={{fontSize:22,fontWeight:500,margin:0,color:BRAND.oroD}}>€ {fmt(totProvvDash)}</p></div>
              <div style={S.card("#27AE60")}><p style={{fontSize:12,color:"#888",margin:"0 0 4px"}}>Incassato</p><p style={{fontSize:22,fontWeight:500,margin:0,color:"#27AE60"}}>€ {fmt(totIncDash)}</p></div>
              <div style={S.card("#E67E22")}><p style={{fontSize:12,color:"#888",margin:"0 0 4px"}}>Da incassare</p><p style={{fontSize:22,fontWeight:500,margin:0,color:"#E67E22"}}>€ {fmt(totProvvDash-totIncDash)}</p></div>
            </div>
            <div style={S.tblWrap}>
              <div style={{padding:"10px 16px",background:"#fafaf8",borderBottom:"0.5px solid #eee",fontSize:13,fontWeight:500,color:"#666"}}>Riepilogo agenti</div>
              <table style={{...S.tbl,minWidth:400}}>
                <thead><tr>{["Agente","Profilo","Venduti","Provv. generate","Incassato","Da incassare"].map(h=><th key={h} style={S.th}>{h}</th>)}</tr></thead>
                <tbody>{agenti.map(ag=>{
                  const vAg=dashVend.filter(v=>v.agenteListing===ag.id||v.agenteAcquirente===ag.id);
                  const gen=vAg.reduce((s,v)=>s+Number(v.provvVenditore||0)+Number(v.provvAcquirente||0),0);
                  const inc=vAg.reduce((s,v)=>s+calcolaIncassatoV(v)+calcolaIncassatoA(v),0);
                  return(<tr key={ag.id}><td style={S.td}><strong>{ag.nome} {ag.cognome}</strong></td><td style={S.td}>{ag.profilo}</td><td style={S.td}>{vAg.length}</td><td style={S.tdR}>€ {fmt(gen)}</td><td style={S.tdR}>€ {fmt(inc)}</td><td style={S.tdR}><span style={{color:gen-inc>0?"#E67E22":"#27AE60",fontWeight:500}}>€ {fmt(gen-inc)}</span></td></tr>);
                })}</tbody>
              </table>
            </div>
            {sospesi.length>0&&(<div style={S.tblWrap}>
              <div style={{padding:"10px 16px",background:"#FEF0E0",borderBottom:"0.5px solid #E67E22",fontSize:13,fontWeight:500,color:"#E67E22",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                <span>💰 Sospesi da incassare — Totale: € {fmt(sospesi.reduce((s,r)=>s+r.importo,0))}</span>
                <span style={{fontSize:11,color:"#aaa",fontWeight:400}}>Clicca una riga per gestire</span>
              </div>
              <table style={{...S.tbl,minWidth:600}}>
                <thead><tr>{["Lato","Agente","Nominativo","Immobile","Già incassato","Residuo","Scadenza",""].map(h=><th key={h} style={S.th}>{h}</th>)}</tr></thead>
                <tbody>{sospesi.map((r,i)=>{
                  const giaIncassato=r.totaleProvv-r.importo;
                  return(<tr key={i} style={{cursor:"pointer"}}
                    onMouseEnter={e=>e.currentTarget.style.background="#FEF6EE"}
                    onMouseLeave={e=>e.currentTarget.style.background=""}
                    onClick={()=>{const v=venduti.find(v=>v.id===r.vendId);if(v)setShowIncassoLato({vend:v,lato:r.lato});}}>
                    <td style={S.td}><span style={{fontSize:11,padding:"2px 8px",borderRadius:4,background:r.lato==="V"?"#EAF4FB":"#F5EEF8",color:r.lato==="V"?"#2980B9":"#8E44AD",fontWeight:500}}>{r.lato==="V"?"Venditore":"Acquirente"}</span></td>
                    <td style={S.td}>{r.agente}</td>
                    <td style={S.td}><strong>{r.nominativo}</strong></td>
                    <td style={S.td}>{r.immobile}</td>
                    <td style={S.td}>{giaIncassato>0?<span style={{fontSize:12,color:"#27AE60"}}>{miniIncasso(r)}</span>:<span style={{fontSize:11,color:"#ccc"}}>Nessun acconto</span>}</td>
                    <td style={{...S.tdR,fontWeight:600,color:"#E67E22"}}>€ {fmt(r.importo)}</td>
                    <td style={S.td}>{r.scadenza?fmtD(r.scadenza):"—"}</td>
                    <td style={S.td}><button style={{...S.btnP,padding:"4px 10px",fontSize:12}} onClick={e=>{e.stopPropagation();const v=venduti.find(v=>v.id===r.vendId);if(v)setShowIncassoLato({vend:v,lato:r.lato});}}>✏️ Gestisci</button></td>
                  </tr>);
                })}</tbody>
              </table>
            </div>)}
            {sospesi.length===0&&totProvvDash>0&&(<div style={{background:"#E9F7EF",border:"1px solid #27AE60",borderRadius:8,padding:"1rem",textAlign:"center",fontSize:13,color:"#27AE60",fontWeight:500}}>✅ Tutti i sospesi sono stati incassati!</div>)}
          </div>)}
          ...
          {tab==="Incarichi"&&(<div style={S.sec}>
            ...
            <FiltriInc/>
            <div style={S.cnt}>
              {[["Attivi",cntInc.attivi,STATI_INC.Attivo.clr],["Scaduti",cntInc.scaduti,STATI_INC.Scaduto.clr],[subInc==="affitto"?"Locati":"Venduti",cntInc.venduti,STATI_INC.Venduto.clr]].map(([l,n,c])=>(
                <div key={l} style={S.cntBox(c)}><span style={{fontSize:24,fontWeight:700,color:c}}>{n}</span><span style={{fontSize:12,color:"#aaa"}}>{l}</span></div>
              ))}
            </div>
            <div style={S.tblWrap}>
              <table style={S.tbl}>
                  ...
                  return(<tr key={inc.id} style={{cursor:"pointer"}} onClick={()=>{setFormInc({...inc,agenteListing:inc.agenteListing||"",buyerListing:inc.buyerListing||""});setShowInc(inc);}}>
                    ...
                    <td style={S.td} onClick={e=>e.stopPropagation()}>
                      <div style={{display:"flex",gap:4}}>
                        {s!=="Venduto"&&s!=="Locato"&&<button style={S.btnG} onClick={()=>{setFormProp(emptyProp(inc.categoria,inc));setShowProp("new");}}>+ Proposta</button>}
                        <button style={S.btnD} onClick={()=>setIncarichi(incarichi.filter(i=>i.id!==inc.id))}>✕</button>
                      </div>
                    </td>
                  </tr>);
                })}
          ...
          {tab==="Proposte"&&(<div style={S.sec}>
            ...
            <FiltriProp/>
            <div style={S.cnt}>
              {[["In attesa",cntProp.attesa,"#4A90D9"],["Con vincolo",cntProp.vincolo,"#D4AC0D"],["Accettate",cntProp.accettate,"#27AE60"],["Non concluse",cntProp.rifiutate,"#E74C3C"]].map(([l,n,c])=>(
                <div key={l} style={S.cntBox(c)}><span style={{fontSize:24,fontWeight:700,color:c}}>{n}</span><span style={{fontSize:12,color:"#aaa"}}>{l}</span></div>
              ))}
            </div>
            <div style={S.tblWrap}>
          ...
          {tab==="Venduti"&&(<div style={S.sec}>
            ...
            <FiltriVend/>
            <div style={S.cnt}>
              {[["Da incassare",cntVend.daIncassare,"#E67E22"],["Parziale",cntVend.parziale,"#D4AC0D"],["Incassato",cntVend.incassato,"#27AE60"]].map(([l,n,c])=>(
                <div key={l} style={S.cntBox(c)}><span style={{fontSize:24,fontWeight:700,color:c}}>{n}</span><span style={{fontSize:12,color:"#aaa"}}>{l}</span></div>
              ))}
            </div>
            <div style={S.tblWrap}>
              <table style={S.tbl}>
                <thead><tr>{["Comune","Indirizzo","Venditore","Acquirente","Ag.L","%L","BuyerL","%BL","Ag.A","%A","Buyer","%B","Prezzo","Provv.V.","Provv.A.","Tipo atto","Data atto","Inc.V.","Inc.A.","Scad.inc.","Stato","Azioni"].map(h=><th key={h} style={S.th}>{h}</th>)}</tr></thead>
                ...
                {vendFiltrati.length>0&&<tfoot><tr style={S.totRow}>
                  <td colSpan={12} style={S.td}>Totale ({vendFiltrati.length})</td>
                  <td style={S.td}/>
          ...
          {tab==="Report Agenti"&&(<div style={S.sec}>
            <div style={S.tblWrap}>
              <table style={{...S.tbl,minWidth:400}}>
                <thead><tr>{["Agente","Profilo","Incarichi","Proposte","Vendite","Provv.Vendite","Inc.Vendite","Provv.Affitti"].map(h=><th key={h} style={S.th}>{h}</th>)}</tr></thead>
                <tbody>{agenti.map(ag=>{
                  const nI=incarichi.filter(i=>i.agenteListing===ag.id).length;
                  const nP=proposte.filter(p=>p.agenteAcquirente===ag.id||p.agenteListing===ag.id).length;
                  const vV=venduti.filter(v=>v.categoria==="vendita"&&(v.agenteListing===ag.id||v.agenteAcquirente===ag.id));
                  const vA=venduti.filter(v=>v.categoria==="affitto"&&(v.agenteListing===ag.id||v.agenteAcquirente===ag.id));
                  const genV=vV.reduce((s,v)=>s+Number(v.provvVenditore||0)+Number(v.provvAcquirente||0),0);
                  const incV=vV.reduce((s,v)=>s+calcolaIncassatoV(v)+calcolaIncassatoA(v),0);
                  const genA=vA.reduce((s,v)=>s+Number(v.provvVenditore||0)+Number(v.provvAcquirente||0),0);
                  return(<tr key={ag.id}><td style={S.td}><strong>{ag.nome} {ag.cognome}</strong></td><td style={S.td}>{ag.profilo}</td><td style={S.td}>{nI}</td><td style={S.td}>{nP}</td><td style={S.td}>{vV.length}</td><td style={S.tdR}>€ {fmt(genV)}</td><td style={S.tdR}>€ {fmt(incV)}</td><td style={S.tdR}>€ {fmt(genA)}</td></tr>);
                })}</tbody>
              </table>
            </div>
          </div>)}
          ...
          {/* ── FATTURA AGENTE ── */}
          {tab==="Fattura Agente"&&(<div style={S.sec}>
            <div style={S.fRow}>
            ...
            {fatAgente&&fatturaDati.length>0&&(
              <div style={{background:"#fff",border:`1px solid ${BRAND.oro}`,borderRadius:10,padding:"1.5rem"}}>
                <div style={{borderBottom:`2px solid ${BRAND.oro}`,paddingBottom:"1rem",marginBottom:"1rem",display:"flex",justifyContent:"space-between",flexWrap:"wrap",gap:8}}>
                  <div style={{lineHeight:1}}><div style={{fontSize:24,fontWeight:700,color:BRAND.grigio,fontFamily:"Georgia,serif"}}>c<span style={{color:BRAND.oro}}>à</span>sa</div><div style={{fontSize:8,letterSpacing:"0.25em",color:BRAND.grigio,borderTop:`1px solid ${BRAND.grigio}`,paddingTop:2,marginTop:1}}>IMMOBILIARE</div></div>
                ...
                {fatturaDati.map(({v,righe,totPratica},i)=>(
                  <div key={v.id} style={{border:"0.5px solid #eee",borderRadius:8,padding:"1rem",marginBottom:"0.75rem",background:BRAND.beige}}>
                    <p style={{fontSize:14,fontWeight:500,margin:"0 0 4px"}}>{i+1}. {v.comuneImmobile} — {v.indirizzoImmobile} <span style={{fontSize:12,color:"#aaa",fontWeight:400}}>({v.tipologia})</span></p>
                    <p style={{fontSize:12,color:"#aaa",margin:"0 0 .5rem"}}>Prezzo: <strong style={{color:BRAND.oroD}}>€ {fmtN(v.prezzoVendita)}</strong></p>
                    {righe.map((r,j)=>(
                      <div key={j} style={{background:"#fff",borderRadius:6,border:"0.5px solid #ddd",padding:"8px 12px",marginBottom:6}}>
                ...
                <div style={{background:BRAND.oro,borderRadius:8,padding:"1rem 1.5rem",display:"flex",justifyContent:"space-between",alignItems:"center",marginTop:"1.25rem"}}>
                  <span style={{fontSize:14,fontWeight:500,color:"#fff"}}>TOTALE IMPONIBILE DA FATTURARE</span>
                  <span style={{fontSize:22,fontWeight:700,color:"#fff"}}>€ {fmt(totImponibile)}</span>
                </div>
              </div>
            )}
          </div>)}
          ...
          {tab==="Agenti"&&(<div style={S.sec}>
            <div style={{display:"flex",justifyContent:"flex-end",marginBottom:"1rem"}}>
              <button style={S.btnP} onClick={()=>{setFormAgente({nome:"",cognome:"",profilo:"Consulente",tipo:"Interno",percListing:0,percAcquirente:0});setShowAgente("new");}}>+ Nuovo agente</button>
            </div>
            <div style={S.tblWrap}><table style={{...S.tbl,minWidth:400}}>
              ...
              <tbody>{agenti.map(a=>(
                <tr key={a.id} style={{cursor:"pointer"}} onClick={()=>{setFormAgente({...a});setShowAgente(a);}}>
                  <td style={S.td}><strong>{a.nome}</strong></td>
                  <td style={S.td}>{a.cognome}</td>
                  <td style={S.td}>
                    <span style={{fontSize:11,padding:"2px 8px",borderRadius:4,fontWeight:500,
                      background:a.profilo==="Broker"?`${BRAND.oro}22`:a.profilo==="Consulente"?"#EAF4FB":"#F0F0F0",
                      color:a.profilo==="Broker"?BRAND.oroD:a.profilo==="Consulente"?"#2980B9":"#666"}}>
                      {a.profilo}
                    </span>
                  </td>
      ...
      {showProp&&(<div style={S.overlay} onClick={e=>e.target===e.currentTarget&&setShowProp(null)}>
        <div style={S.modal}>
          ...
          {formProp.tipo==="collaborazione"&&(<div style={S.hl}>
            <div style={S.g2}>
          ...
          <div style={{display:"flex",alignItems:"center",gap:8,margin:"8px 0"}}>
            <input type="checkbox" id="vinc" checked={formProp.vincolata||false} onChange={e=>setFormProp({...formProp,vincolata:e.target.checked})}/>
            <label htmlFor="vinc" style={{fontSize:13}}>Proposta vincolata</label>
          </div>
          ...
          <div style={{display:"flex",gap:8,justifyContent:"flex-end",marginTop:"1rem"}}>
            <button style={S.btn} onClick={()=>setShowProp(null)}>Annulla</button>
            <button style={S.btnP} onClick={salvaProp}>Salva</button>
          </div>
        </div>
      </div>)}
        ...
        <div style={S.modal}>
          ...
          <div style={{marginBottom:12}}>
            ...
            <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
              {["Accettata","Controproposta","Rifiutata","Mancata Chiusura",...(showGestProp.vincolata?["Accettata con Vincolo"]:[])].map(s=>{
                const cfg=STATI_PROP[s]; const sel=formStatoProp.stato===s;
                return(<button key={s} onClick={()=>setFormStatoProp({...formStatoProp,stato:s})} style={{...S.btn,border:`1.5px solid ${sel?cfg?.clr:"#ddd"}`,background:sel?cfg?.bg:"#fff",color:sel?cfg?.clr:BRAND.grigio,fontWeight:sel?500:400}}>{cfg?.s} {s}</button>);
              })}
            </div>
          </div>
          ...
          <div style={{display:"flex",gap:8,justifyContent:"flex-end",marginTop:"1rem"}}>
            ...
            <button style={S.btnP} onClick={salvaStatoProp}>Conferma</button>
          </div>
        </div>
      </div>)}
      ...
      {showGestVend&&(<div style={S.overlay} onClick={e=>e.target===e.currentTarget&&setShowGestVend(null)}>
        <div style={S.modal}>
          ...
          <div style={S.hl}>
            <p style={{fontSize:13,fontWeight:500,margin:"0 0 8px"}}>Provvigioni</p>
            <div style={S.g2}>
              <div><label style={S.lbl}>Provv. venditore (€)</label><input style={S.inp} type="number" value={formVend.provvVenditore||0} onChange={e=>setFormVend({...formVend,provvVenditore:Number(e.target.value)})}/></div>
              <div><label style={S.lbl}>Provv. acquirente (€)</label><input style={S.inp} type="number" value={formVend.provvAcquirente||0} onChange={e=>setFormVend({...formVend,provvAcquirente:Number(e.target.value)})}/></div>
            </div>
          </div>
          <div style={S.g2}>
            <div><label style={S.lbl}>Tipo atto</label><select style={S.inp} value={formVend.tipoAtto||"Preliminare"} onChange={e=>setFormVend({...formVend,tipoAtto:e.target.value})}><option>Preliminare</option><option>Rogito Diretto</option><option>Rogito</option></select></div>
            <div><label style={S.lbl}>Data atto</label><input style={S.inp} type="date" value={formVend.dataAtto||""} onChange={e=>setFormVend({...formVend,dataAtto:e.target.value})}/></div>
          </div>
          <div style={{marginBottom:"1rem"}}>
            <label style={S.lbl}>Scadenza incasso</label>
            <input style={{...S.inp,maxWidth:200}} type="date" value={formVend.scadenzaIncasso||""} onChange={e=>setFormVend({...formVend,scadenzaIncasso:e.target.value})}/>
          </div>
          <p style={{fontSize:12,color:"#aaa",fontStyle:"italic",margin:"0 0 1rem"}}>💡 Per gestire acconti e saldo usa i pulsanti V / A nella tabella Venduti</p>
          <div style={{display:"flex",gap:8,justifyContent:"flex-end",marginTop:"1rem"}}>
            <button style={S.btn} onClick={()=>setShowGestVend(null)}>Annulla</button>
            <button style={S.btnP} onClick={salvaVend}>Salva</button>
          </div>
        </div>
      </div>)}
      ...
      {showIncassoLato&&(
        <ModalIncassoLato
          vend={showIncassoLato.vend}
          lato={showIncassoLato.lato}
          onSave={handleSaveIncassoLato}
          onClose={()=>setShowIncassoLato(null)}
        />
      )}
      ...
      {showAgente&&(<div style={S.overlay} onClick={e=>e.target===e.currentTarget&&setShowAgente(null)}>
        <div style={{...S.modal,width:"min(96vw,480px)"}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:"1.25rem"}}>
            <h2 style={{fontSize:17,fontWeight:500,margin:0}}>{showAgente==="new"?"Nuovo agente":"Modifica agente"}</h2>
            <button onClick={()=>setShowAgente(null)} style={{background:"none",border:"none",fontSize:20,cursor:"pointer",color:"#ccc",lineHeight:1,padding:0}}>✕</button>
          </div>
          <div style={S.g2}>
            <div><label style={S.lbl}>Nome</label><input style={S.inp} value={formAgente.nome||""} onChange={e=>setFormAgente({...formAgente,nome:e.target.value})}/></div>
            <div><label style={S.lbl}>Cognome</label><input style={S.inp} value={formAgente.cognome||""} onChange={e=>setFormAgente({...formAgente,cognome:e.target.value})}/></div>
          </div>
          <div style={S.g2}>
            <div><label style={S.lbl}>Profilo</label>
              <select style={S.inp} value={formAgente.profilo||"Consulente"} onChange={e=>setFormAgente({...formAgente,profilo:e.target.value,percListing:e.target.value==="Broker"?0:formAgente.percListing,percAcquirente:e.target.value==="Broker"?0:formAgente.percAcquirente})}>
                <option>Broker</option><option>Consulente</option><option>Collaboratore</option>
              </select>
            </div>
            <div><label style={S.lbl}>Tipo</label>
              <select style={S.inp} value={formAgente.tipo||"Interno"} onChange={e=>setFormAgente({...formAgente,tipo:e.target.value})}>
                <option>Interno</option><option>Esterno</option>
              </select>
            </div>
          </div>
          {formAgente.profilo!=="Broker"&&(
            <div style={S.g2}>
              <div><label style={S.lbl}>% Provv. Listing</label><input style={S.inp} type="number" min="0" max="100" step="0.5" value={formAgente.percListing||0} onChange={e=>setFormAgente({...formAgente,percListing:Number(e.target.value)})}/></div>
              <div><label style={S.lbl}>% Provv. Acquirente</label><input style={S.inp} type="number" min="0" max="100" step="0.5" value={formAgente.percAcquirente||0} onChange={e=>setFormAgente({...formAgente,percAcquirente:Number(e.target.value)})}/></div>
            </div>
          )}
          <div style={{display:"flex",gap:8,justifyContent:"flex-end",marginTop:"1.25rem"}}>
            <button style={S.btn} onClick={()=>setShowAgente(null)}>Annulla</button>
            <button style={S.btnP} onClick={()=>{
              if(!formAgente.nome||!formAgente.cognome) return;
              if(showAgente==="new"){
                setAgenti([...agenti,{...formAgente,id:Date.now()}]);
              } else {
                setAgenti(agenti.map(a=>a.id===showAgente.id?{...formAgente,id:a.id}:a));
              }
              setShowAgente(null);
            }}>Salva</button>
          </div>
