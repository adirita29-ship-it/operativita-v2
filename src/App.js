...
// Carica la libreria Supabase con un tag <script>. Prima si usava import()
// dinamico, che il compilatore di produzione non accetta sempre.
const caricaLibSupabase = () => new Promise((ok, ko) => {
  if(window.supabase && window.supabase.createClient) return ok(window.supabase);
  const gia = document.getElementById("lib-supabase-js");
  if(gia){
    gia.addEventListener("load", ()=> window.supabase ? ok(window.supabase) : ko(new Error("libreria non disponibile")));
    gia.addEventListener("error", ()=> ko(new Error("caricamento libreria non riuscito")));
    return;
  }
  const s = document.createElement("script");
  s.id = "lib-supabase-js";
  s.src = "https://cdn.jsdelivr.net/npm/@supabase/supabase-js@2";
  s.async = true;
  s.onload = ()=> window.supabase ? ok(window.supabase) : ko(new Error("libreria non disponibile"));
  s.onerror = ()=> ko(new Error("caricamento libreria non riuscito"));
...
const caricaDB = async () => {
  const res = await fetch(`${SUPA_URL}/rest/v1/gestionale_data?id=eq.main&select=data`,
    {headers:authHeaders()});
  if(!res.ok) throw new Error("Lettura dati non riuscita (HTTP "+res.status+")");
  const rows = await res.json();
...
const salvaDBMerge = async (data) => {
  let remote=null, letturaKo=false;
  try { remote = await caricaDB(); } catch(e){ remote=null; letturaKo=true; }
  if(remote===null && letturaKo){
    // 2° tentativo: se la lettura fallisce non devo sovrascrivere il server col locale
    try { remote = await caricaDB(); letturaKo=false; } catch(e){ remote=null; letturaKo=true; }
  }
  if(letturaKo){
    console.error("Salvataggio annullato: il server non risponde alla lettura.");
    if(typeof window!=="undefined"){ window.__ultimoErroreSalvataggio={timestamp:Date.now(),errore:"lettura fallita"}; }
    alert("⚠ Salvataggio non riuscito: il server non risponde.\nLe modifiche restano su questo dispositivo. Ricarica la pagina fra poco.");
    return;
  }
  try {
...
export default function App() {
  ...
  useEffect(()=>{
    ...
    caricaDB().then(data=>{
      ...
      setErroreCaricamento(String(e?.message||e));
    });
  },[sessionePronta]);
  ...
  useEffect(()=>{
    ...
    const initRealtime=async()=>{
      try{
        const {createClient}=await caricaLibSupabase();
  ...
  },[agenti,incarichi,proposte,venduti,archiviati,archiviatiProp,archiviatiVend,fonti,tipologie,vincoli,tipiNeg,tipiVolantino,tipiSviluppo,operativita,obiettiviOp,pratiche,pagamentiFatture,prospetti,ericaTodo,agenteTodo,costi,obiettivoFatturato,obiettivoQuotaAgenzia,obiettivoAgente,provvStandard,costiAgente,mirino,sfide,notizie,oneToOne,fasiConfig,emailLog,catCosti,speseCosti,breakEvenManuale,catalogoAzioni,routineProf,oggiDati,volantinaggi,eventi,tipiEvento,tracciamento,dbLoaded]);
  ...
  return(
    <div style={{display:"flex",height:"100vh",background:BRAND.beige,overflow:"hidden",fontFamily:"'Georgia',serif",color:BRAND.grigio}}>
      ...
      <div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden"}}>
        ...
        {erroreCaricamento&&(
          <div style={{background:"#FDEDEC",borderBottom:"1px solid #E6B0AA",padding:"8px 14px",fontSize:12,color:"#943126",flexShrink:0}}>
            ⚠ I dati non sono stati caricati dal server ({erroreCaricamento}). <strong>Le modifiche fatte ora non verranno salvate.</strong> Ricarica la pagina; se il problema resta, avvisa il broker.
          </div>
        )}
