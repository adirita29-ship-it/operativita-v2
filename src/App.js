        ...
        <div style={{flex:1,overflowY:"auto"}}>
          ...
          {tab==="Dashboard"&&(<div style={S.sec}>
            ...
            {!isBroker&&(()=>{
              ...
              const sfidaAttiva=sfide.find(s=>s.dal<=todayStr()&&s.al>=todayStr()&&!s.conclusa);
              const METRICHE={acquisizioni:"🏠 Acquisizioni",fatturato:"💰 Fatturato",chiamate:"📞 Chiamate",oh:"🚪 Open House",proposte:"📝 Proposte"};
              const calcM=(agId,metrica,d1,d2)=>{
                const incP=incarichi.filter(i=>i.agenteListing===agId&&i.dataInizio>=d1&&i.dataInizio<=d2);
                const propP=proposte.filter(p=>(p.agenteListing===agId||p.agenteAcquirente===agId)&&(p.dataStato||"")>=d1&&(p.dataStato||"")<=d2);
                ...
                switch(metrica){case "acquisizioni":return incP.length;case "fatturato":return vendP.reduce((s,v)=>s+Number(v.provvVenditore||0)+Number(v.provvAcquirente||0),0);case "chiamate":return ch;case "oh":return gg.reduce((s,[,g])=>s+(g.ohImmobili||[]).length,0);case "proposte":return propP.length;default:return 0;}
              };
              const myRogiti=venduti.filter(v=>{if(!v.dataAtto||(v.agenteListing!==myAgentId&&v.agenteAcquirente!==myAgentId))return false;const d=toD(v.dataAtto);return d>=oggiD&&d<=tra30;}).sort((a,b)=>a.dataAtto.localeCompare(b.dataAtto));
              const myAlerts=incarichi.filter(i=>!i.archiviato&&i.agenteListing===myAgentId).map(i=>({inc:i,al:getAlert(i.id)})).filter(x=>x.al.length>0);
              const PODIO=["🥇","🥈","🥉","4°","5°"];
              const PODIO_CLR=["#D4AC0D","#888","#CD7F32","#555","#777"];
              return(<div style={{display:"grid",gridTemplateColumns:isMobile?"1fr":sfidaAttiva?"1fr 1fr 1fr":"1fr 1fr",gap:10,marginTop:"1rem"}}>
                {sfidaAttiva&&(()=>{
                  const cl=agenti.map(ag=>({ag,val:calcM(ag.id,sfidaAttiva.metrica,sfidaAttiva.dal,sfidaAttiva.al)})).sort((a,b)=>b.val-a.val);
                  const miaPos=cl.findIndex(x=>x.ag.id===myAgentId);
                  const mioVal=cl[miaPos]?.val||0;
                  const ggR=Math.max(0,Math.round((toD(sfidaAttiva.al)-oggiD)/86400000));
                  return(<div style={{background:"linear-gradient(135deg,#FDF6EC,#FAEEDA)",borderRadius:10,border:"1px solid #D4AC0D44",padding:"1rem"}}>
                    <div style={{display:"flex",justifyContent:"space-between",marginBottom:8}}>
                      <div><div style={{fontSize:11,fontWeight:600,color:"#D4AC0D",marginBottom:2}}>🏆 {sfidaAttiva.nome}</div><div style={{fontSize:10,color:"#aaa"}}>{METRICHE[sfidaAttiva.metrica]} · 🎁 {sfidaAttiva.premio}</div></div>
                    ...
                    <div style={{background:"#fff",borderRadius:8,padding:"8px",marginBottom:8,textAlign:"center"}}>
                      <div style={{fontSize:20}}>{PODIO[miaPos]||"—"}</div>
                      <div style={{fontSize:18,fontWeight:700,color:PODIO_CLR[miaPos]||"#555"}}>{sfidaAttiva.metrica==="fatturato"?`€ ${fmt(mioVal)}`:mioVal}</div>
                      <div style={{fontSize:10,color:"#aaa"}}>Sei {miaPos+1}° su {agenti.length}</div>
                    </div>
                    {cl.slice(0,3).map(({ag,val},i)=>(<div key={ag.id} style={{display:"flex",justifyContent:"space-between",fontSize:11,padding:"2px 6px",borderRadius:3,background:ag.id===myAgentId?"#FEF0E0":"transparent",fontWeight:ag.id===myAgentId?600:400}}>
                      <span>{PODIO[i]} {ag.nome}</span><span style={{color:PODIO_CLR[i]}}>{sfidaAttiva.metrica==="fatturato"?`€ ${fmt(val)}`:val}</span>
                    </div>))}
                  </div>);
                })()}
                <div style={{background:"#fff",borderRadius:10,border:"0.5px solid #e8e5e0",padding:"1rem"}}>
                  ...
                  {myRogiti.length===0?<p style={{fontSize:12,color:"#bbb",textAlign:"center",padding:"0.5rem 0"}}>Nessun rogito nei prossimi 30 giorni</p>
                  :myRogiti.map(v=>{const gg=Math.round((toD(v.dataAtto)-oggiD)/86400000);return(<div key={v.id} style={{padding:"6px 0",borderBottom:"0.5px solid #f5f5f5"}}>
                    <div style={{fontSize:12,fontWeight:500}}>{v.comuneImmobile} — {v.indirizzoImmobile}</div>
                    <div style={{display:"flex",justifyContent:"space-between",marginTop:2}}><span style={{fontSize:11,color:"#888"}}>{v.nominativoVenditore}</span><span style={{fontSize:11,fontWeight:600,color:gg<=7?"#E74C3C":gg<=15?"#E67E22":"#27AE60"}}>{gg===0?"Oggi!":gg===1?"Domani":gg+" gg"}</span></div>
                  </div>);})}
                </div>
                <div style={{background:"#fff",borderRadius:10,border:`0.5px solid ${myAlerts.length>0?"#E74C3C44":"#e8e5e0"}`,padding:"1rem"}}>
                  <p style={{fontSize:11,fontWeight:600,color:myAlerts.length>0?"#E74C3C":"#888",textTransform:"uppercase",letterSpacing:".08em",margin:"0 0 8px"}}>{myAlerts.length>0?"⚠ Alert pratiche":"✅ Pratiche RT"}{myAlerts.length>0&&<span style={{marginLeft:6,fontSize:10,padding:"1px 6px",borderRadius:3,background:"#FDECEA"}}>{myAlerts.length}</span>}</p>
                  {myAlerts.length===0?<p style={{fontSize:12,color:"#bbb",textAlign:"center",padding:"0.5rem 0"}}>Tutto in regola!</p>
                  :myAlerts.slice(0,3).map(({inc,al})=>(<div key={inc.id} style={{padding:"6px 0",borderBottom:"0.5px solid #f5f5f5"}}><div style={{fontSize:12,fontWeight:500}}>{inc.comune} — {inc.indirizzo}</div><div style={{fontSize:11,color:"#E74C3C",marginTop:2}}>{al[0].lbl}{al.length>1?` +${al.length-1}`:""}</div></div>))}
                </div>
              </div>);
            })()}
            ...
            {/* Prossimi Rogiti + Alert Pratiche */}
            {(()=>{
              const oggiD=new Date(); oggiD.setHours(0,0,0,0);
              const tra30=new Date(oggiD); tra30.setDate(tra30.getDate()+30);
              const toD=s=>{const d=new Date(s);d.setHours(0,0,0,0);return d;};
              const prossimiRogiti=venduti.filter(v=>{if(!v.dataAtto)return false;const d=toD(v.dataAtto);return d>=oggiD&&d<=tra30;}).sort((a,b)=>a.dataAtto.localeCompare(b.dataAtto));
              const alertPratiche=incarichi.filter(i=>!i.archiviato&&statoInc(i)!=="Venduto").map(i=>({inc:i,al:getAlert(i.id)})).filter(x=>x.al.length>0);
              return(<div style={{display:"grid",gridTemplateColumns:isMobile?"1fr":"1fr 1fr",gap:10,marginTop:"1rem"}}>
                <div style={{background:"#fff",borderRadius:10,border:"0.5px solid #e8e5e0",padding:"1rem"}}>
                  ...
                  {prossimiRogiti.length===0?<p style={{fontSize:12,color:"#bbb",textAlign:"center",padding:"0.5rem 0"}}>Nessun rogito nei prossimi 30 giorni</p>
                  :prossimiRogiti.map(v=>{const gg=Math.round((toD(v.dataAtto)-oggiD)/86400000);const ag=agenti.find(a=>a.id===v.agenteListing);return(<div key={v.id} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"7px 0",borderBottom:"0.5px solid #f5f5f5",flexWrap:"wrap",gap:4}}>
                ...
                <div style={{background:"#fff",borderRadius:10,border:`0.5px solid ${alertPratiche.length>0?"#E74C3C44":"#e8e5e0"}`,padding:"1rem"}}>
                  <p style={{fontSize:11,fontWeight:600,color:alertPratiche.length>0?"#E74C3C":"#888",textTransform:"uppercase",letterSpacing:".08em",margin:"0 0 10px"}}>
                    {alertPratiche.length>0?"⚠ Alert pratiche RT":"✅ Pratiche RT"}
                    {alertPratiche.length>0&&<span style={{marginLeft:6,padding:"1px 6px",borderRadius:3,background:"#FDECEA",color:"#E74C3C",fontSize:10}}>{alertPratiche.length}</span>}
                  </p>
                  {alertPratiche.length===0?<p style={{fontSize:12,color:"#bbb",textAlign:"center",padding:"0.5rem 0"}}>Nessun alert — tutto in regola</p>
                  :alertPratiche.slice(0,4).map(({inc,al})=>(
                    <div key={inc.id} style={{padding:"7px 0",borderBottom:"0.5px solid #f5f5f5"}}>
                      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:3}}>
                        <div style={{fontSize:12,fontWeight:500}}>{inc.comune} — {inc.indirizzo}</div>
                        <span style={{fontSize:10,padding:"1px 6px",borderRadius:3,background:"#FDECEA",color:"#E74C3C",fontWeight:600}}>{al.length} alert</span>
                      </div>
                      <div style={{fontSize:11,color:"#E74C3C"}}>{al[0].lbl}</div>
                      {al.length>1&&<div style={{fontSize:10,color:"#aaa"}}>+{al.length-1} altri</div>}
                    </div>
                  ))}
                  {alertPratiche.length>4&&<div style={{fontSize:11,color:"#aaa",textAlign:"center",marginTop:6}}>+{alertPratiche.length-4} altre</div>}
                </div>
              </div>);
            })()}
            </>)}
          ...
          {tab==="War Room"&&(()=>{
            ...
            const [dal,al]=getPeriodo();

            const calcMetrica=(agId,metrica,d1,d2)=>{
              const incP=incarichi.filter(i=>i.agenteListing===agId&&i.dataInizio>=d1&&i.dataInizio<=d2);
              const propP=proposte.filter(p=>(p.agenteListing===agId||p.agenteAcquirente===agId)&&(p.dataStato||"")>=d1&&(p.dataStato||"")<=d2);
              const vendP=venduti.filter(v=>(v.agenteListing===agId||v.agenteAcquirente===agId)&&(v.dataAtto||"")>=d1&&(v.dataAtto||"")<=d2);
              const gg=Object.entries(operativita[agId]||{}).filter(([d])=>d>=d1&&d<=d2);
            ...
            const prodAgenti=agenti.map(ag=>{
              const acq=incarichi.filter(i=>i.agenteListing===ag.id&&i.dataInizio>=dal&&i.dataInizio<=al).length;
              const prop=proposte.filter(p=>(p.agenteListing===ag.id||p.agenteAcquirente===ag.id)&&(p.dataStato||"")>=dal&&(p.dataStato||"")<=al).length;
              const vendP=venduti.filter(v=>(v.agenteListing===ag.id||v.agenteAcquirente===ag.id)&&(v.dataAtto||"")>=dal&&(v.dataAtto||"")<=al);
              const fatt=vendP.reduce((s,v)=>s+Number(v.provvVenditore||0)+Number(v.provvAcquirente||0),0);
              const gg=Object.entries(operativita[ag.id]||{}).filter(([d])=>d>=dal&&d<=al);
              const ch=gg.reduce((s,[,g])=>{const ct=g.chiamate_tipi||{};return s+Object.values(ct).reduce((a,v)=>a+Number(v||0),0);},0);
              const giorniComp=gg.filter(([,g])=>Object.values(g).some(v=>Number(v||0)>0)).length;
              return{ag,acq,prop,vend:vendP.length,fatt,chiamate:ch,giorniComp};
            });

            const mesiAnnoW=Array.from({length:12},(_,i)=>`${warAnno}-${String(i+1).padStart(2,"0")}`);
            const obFattTeam=agenti.reduce((s,ag)=>{const maxOb=Math.max(0,...mesiAnnoW.map(m=>{const ob=(obiettiviOp[ag.id]||{})[m]||{};const p=ob.proposti||ob||{};return Number(p.fatturato||0);}));return s+(maxOb>0?maxOb*12:0);},0)||obiettivoFatturato||0;
            const fattAnno=venduti.filter(v=>getAnno(dataCompAgenzia(v))===warAnno).reduce((s,v)=>s+Number(v.provvVenditore||0)+Number(v.provvAcquirente||0),0);
            const percFatt=obFattTeam>0?Math.min(100,Math.round(fattAnno/obFattTeam*100)):0;
            const sez={background:"#fff",borderRadius:12,border:"0.5px solid #e8e5e0",padding:"1rem 1.25rem",marginBottom:"1rem"};
            ...
            return(<div style={{...S.sec,...(warRiunione?{position:"fixed",inset:0,zIndex:1000,background:"#F5F3EE",overflowY:"auto",padding:"1.5rem"}:{})}}>
              ...
              <div style={{fontSize:11,color:"#aaa",marginBottom:"1rem"}}>Periodo: {fmtD(dal)} → {fmtD(al)}</div>

              {/* Termometro */}
              <div style={{background:"linear-gradient(135deg,#1a1a2e,#16213e)",borderRadius:12,padding:"1.25rem 1.5rem",marginBottom:"1rem",color:"#fff"}}>
                <p style={{fontSize:11,fontWeight:600,textTransform:"uppercase",letterSpacing:".1em",color:"#aaa",margin:"0 0 8px"}}>🎯 Obiettivo team {warAnno}</p>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-end",marginBottom:12,flexWrap:"wrap",gap:8}}>
                  <div><div style={{fontSize:warRiunione?36:28,fontWeight:700}}>€ {fmt(fattAnno)}</div><div style={{fontSize:12,color:"#aaa"}}>su obiettivo € {fmt(obFattTeam)}</div></div>
                  <div style={{textAlign:"right"}}><div style={{fontSize:warRiunione?48:32,fontWeight:700,color:percFatt>=100?"#27AE60":percFatt>=50?"#D4AC0D":"#E74C3C"}}>{percFatt}%</div>{percFatt<100&&<div style={{fontSize:12,color:"#aaa"}}>mancano € {fmt(obFattTeam-fattAnno)}</div>}</div>
                </div>
                <div style={{height:warRiunione?28:20,background:"rgba(255,255,255,.1)",borderRadius:14,overflow:"hidden"}}>
                  <div style={{height:"100%",width:`${percFatt}%`,background:percFatt>=100?"#27AE60":percFatt>=50?"linear-gradient(90deg,#D4AC0D,#E67E22)":"linear-gradient(90deg,#E74C3C,#C0392B)",borderRadius:14,display:"flex",alignItems:"center",justifyContent:"flex-end",paddingRight:12}}>
                    {percFatt>15&&<span style={{fontSize:warRiunione?16:12,fontWeight:600,color:"#fff"}}>{percFatt}%</span>}
                  </div>
                </div>
              </div>
              ...
              {/* Traguardo volante */}
              {sfidaAttiva&&(()=>{
                const ggR=Math.max(0,Math.round((new Date(sfidaAttiva.al)-new Date())/86400000));
                const cl=agenti.map(ag=>({ag,val:calcMetrica(ag.id,sfidaAttiva.metrica,sfidaAttiva.dal,sfidaAttiva.al)})).sort((a,b)=>b.val-a.val);
                return(<div style={{...sez,border:"2px solid #D4AC0D"}}>
                  <div style={{display:"flex",justifyContent:"space-between",marginBottom:"1rem",flexWrap:"wrap",gap:8}}>
                    <div><div style={{fontSize:warRiunione?20:16,fontWeight:700,color:"#D4AC0D",marginBottom:4}}>🏆 {sfidaAttiva.nome}</div><p style={{fontSize:12,color:"#888",margin:0}}>{METRICHE[sfidaAttiva.metrica]} · {fmtD(sfidaAttiva.dal)}→{fmtD(sfidaAttiva.al)} · 🎁 {sfidaAttiva.premio}</p></div>
                    <div style={{textAlign:"right",flexShrink:0}}><div style={{fontSize:10,color:"#aaa"}}>Scade tra</div><div style={{fontSize:warRiunione?32:24,fontWeight:700,color:ggR<7?"#E74C3C":"#E67E22"}}>{ggR} gg</div></div>
                  </div>
                  <div style={{display:"grid",gridTemplateColumns:`repeat(${Math.min(agenti.length,4)},1fr)`,gap:8}}>
                    {cl.slice(0,4).map(({ag,val},i)=>(
                      <div key={ag.id} style={{background:i===0?"linear-gradient(135deg,#FDF6EC,#FAEEDA)":"#fafaf8",borderRadius:10,padding:warRiunione?"16px":"12px",textAlign:"center",border:i===0?"1px solid #D4AC0D":"0.5px solid #eee"}}>
                        <div style={{fontSize:warRiunione?32:24,marginBottom:4}}>{PODIO_EMOJI[i]}</div>
                        <div style={{width:warRiunione?40:30,height:warRiunione?40:30,borderRadius:"50%",background:PODIO_CLR[i],display:"flex",alignItems:"center",justifyContent:"center",fontSize:warRiunione?16:12,fontWeight:700,color:"#fff",margin:"0 auto 6px"}}>{ag.nome.charAt(0)}</div>
                        <div style={{fontSize:warRiunione?15:12,fontWeight:500}}>{ag.nome}</div>
                        <div style={{fontSize:warRiunione?26:20,fontWeight:700,color:PODIO_CLR[i]}}>{sfidaAttiva.metrica==="fatturato"?`€ ${fmt(val)}`:val}</div>
                      </div>
                    ))}
                  </div>
                  {isBroker&&<button style={{...S.btnD,fontSize:11,marginTop:10}} onClick={()=>setSfide(sfide.map(s=>s===sfidaAttiva?{...s,conclusa:true}:s))}>Concludi sfida</button>}
                </div>);
              })()}

              {/* Produzione agenti */}
              <div style={sez}>
                <p style={{fontSize:11,fontWeight:600,color:"#888",textTransform:"uppercase",letterSpacing:".08em",margin:"0 0 12px"}}>📊 Produzione nel periodo</p>
                <div style={{overflowX:"auto"}}>
                  <table style={{width:"100%",borderCollapse:"collapse",fontSize:warRiunione?14:12,minWidth:500}}>
                    <thead><tr style={{background:"#fafal8"}}>
                      {["Agente","📅 Giorni","📞 Chiamate","🏠 Acq.","📝 Prop.","✅ Vend.","💰 Fatturato"].map(h=><th key={h} style={{...S.th,textAlign:"center",padding:warRiunione?"12px":"8px 10px"}}>{h}</th>)}
                    </tr></thead>
                    <tbody>{prodAgenti.map(({ag,acq,prop,vend,fatt,chiamate,giorniComp})=>(
                      <tr key={ag.id} style={{borderBottom:"0.5px solid #f5f5f5"}}>
                        <td style={{padding:warRiunione?"12px":"8px 10px"}}><div style={{display:"flex",alignItems:"center",gap:6}}><div style={{width:warRiunione?32:24,height:warRiunione?32:24,borderRadius:"50%",background:`linear-gradient(135deg,${BRAND.oro},#A8863A)`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:warRiunione?14:10,fontWeight:700,color:"#fff"}}>{ag.nome.charAt(0)}</div><span style={{fontWeight:500,fontSize:warRiunione?15:12}}>{ag.nome} {ag.cognome}</span></div></td>
                        <td style={{...S.tdC}}><span style={{fontSize:warRiunione?14:11,padding:"2px 6px",borderRadius:3,background:giorniComp>=5?"#E9F7EF":giorniComp>=3?"#FEF9E7":"#FCEBEB",color:giorniComp>=5?"#27AE60":giorniComp>=3?"#D4AC0D":"#E74C3C",fontWeight:500}}>{giorniComp}</span></td>
                        <td style={{...S.tdC,fontWeight:500,color:"#185FA5",fontSize:warRiunione?15:12}}>{chiamate||"—"}</td>
                        <td style={{...S.tdC,fontWeight:500,color:"#533AB7",fontSize:warRiunione?15:12}}>{acq||"—"}</td>
                        <td style={{...S.tdC,fontSize:warRiunione?15:12}}>{prop||"—"}</td>
                        <td style={{...S.tdC,fontWeight:500,color:"#8E44AD",fontSize:warRiunione?15:12}}>{vend||"—"}</td>
                        <td style={{...S.tdR,fontWeight:600,color:BRAND.oroD,fontSize:warRiunione?15:12}}>{fatt>0?`€ ${fmt(fatt)}`:"—"}</td>
                      </tr>
                    ))}</tbody>
                  </table>
                </div>
              ...
              {/* Storico sfide */}
              {sfideStorico.length>0&&!warRiunione&&(<div style={sez}>
                <p style={{fontSize:11,fontWeight:600,color:"#888",textTransform:"uppercase",letterSpacing:".08em",margin:"0 0 10px"}}>Storico traguardi</p>
                {sfideStorico.map((s,i)=>{const cl=agenti.map(ag=>({ag,val:calcMetrica(ag.id,s.metrica,s.dal,s.al)})).sort((a,b)=>b.val-a.val);const w=cl[0];return(<div key={i} style={{display:"flex",justifyContent:"space-between",padding:"7px 0",borderBottom:"0.5px solid #f5f5f5",flexWrap:"wrap",gap:6}}><div><span style={{fontSize:12,fontWeight:500}}>{s.nome}</span><span style={{fontSize:11,color:"#aaa",marginLeft:8}}>{METRICHE[s.metrica]} · {fmtD(s.dal)}–{fmtD(s.al)}</span></div><div style={{display:"flex",gap:8}}>{w&&w.val>0&&<span style={{fontSize:12,color:"#D4AC0D",fontWeight:600}}>🥇 {w.ag.nome}: {s.metrica==="fatturato"?`€ ${fmt(w.val)}`:w.val}</span>}<span style={{fontSize:11,color:"#aaa"}}>🎁 {s.premio}</span></div></div>);})}
              </div>)}

              {/* Form crea sfida */}
              {isBroker&&!warRiunione&&(<div style={sez}>
                <p style={{fontSize:11,fontWeight:600,color:"#888",textTransform:"uppercase",letterSpacing:".08em",margin:"0 0 12px"}}>⚡ Crea traguardo volante</p>
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:10}}>
                  <div><label style={{fontSize:11,color:"#888",display:"block",marginBottom:3}}>Nome sfida</label><input style={S.inp} value={formSfida.nome} placeholder="es. Maggio Sprint" onChange={e=>setFormSfida({...formSfida,nome:e.target.value})}/></div>
                  <div><label style={{fontSize:11,color:"#888",display:"block",marginBottom:3}}>Metrica</label><select style={S.sel} value={formSfida.metrica} onChange={e=>setFormSfida({...formSfida,metrica:e.target.value})}>{Object.entries(METRICHE).map(([v,l])=><option key={v} value={v}>{l}</option>)}</select></div>
                  <div><label style={{fontSize:11,color:"#888",display:"block",marginBottom:3}}>Dal</label><input type="date" style={S.inp} value={formSfida.dal} onChange={e=>setFormSfida({...formSfida,dal:e.target.value})}/></div>
                  <div><label style={{fontSize:11,color:"#888",display:"block",marginBottom:3}}>Al</label><input type="date" style={S.inp} value={formSfida.al} onChange={e=>setFormSfida({...formSfida,al:e.target.value})}/></div>
                </div>
                <div style={{marginBottom:10}}><label style={{fontSize:11,color:"#888",display:"block",marginBottom:3}}>Premio 🎁</label><input style={S.inp} value={formSfida.premio} placeholder="es. Cena al ristorante, buono €100..." onChange={e=>setFormSfida({...formSfida,premio:e.target.value})}/></div>
                <button style={{...S.btnP,width:"100%",padding:9}} onClick={()=>{if(!formSfida.nome||!formSfida.al){alert("Inserisci nome e data fine");return;}setSfide([...sfide,{...formSfida,id:Date.now(),conclusa:false}]);setFormSfida({nome:"",metrica:"acquisizioni",dal:todayStr(),al:"",premio:""});}}>🏆 Avvia traguardo volante</button>
              </div>)}
            </div>);
          })()}
